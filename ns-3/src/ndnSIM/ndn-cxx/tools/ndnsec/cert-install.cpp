...
class HttpException : public std::runtime_error {
  public:
    explicit HttpException(const std::string& what)
...
ndnsec_cert_install(int argc, char** argv)
{
    ...
    po::options_description description("Usage: ndnsec cert-install [-h] [-I|-K|-N] [-f] FILE\n"
                                        "\n"
                                        "Options");
    description
      .add_options()("help,h", "produce help message")(
        ...
        "using a plain HTTP/1.0 GET request")("identity-default,I", po::bool_switch(&isIdentityDefault),
                                              "set the imported certificate as the default certificate for the "
                                              "identity")("key-default,K", po::bool_switch(&isKeyDefault),
                                                          "set the imported certificate as the default certificate for "
                                                          "the key")("no-default,N", po::bool_switch(&isNoDefault),
    ...
    if (isIdentityDefault + isKeyDefault + isNoDefault > 1) {
        std::cerr << "ERROR: at most one of '--identity-default', '--key-default', "
                     "or '--no-default' may be specified"
                  << std::endl;
        return 2;
    }
