...
ndnsec_get_default(int argc, char** argv)
{
    ...
    po::options_description description("Usage: ndnsec get-default [-h] [-k|-c] [-i ID|-K KEY] [-q]\n"
                                        "\n"
                                        "Options");
    description
      .add_options()(
        "help,h",
        "produce help message")("default-key,k", po::bool_switch(&wantDefaultKey),
                                "show default key, instead of identity")("default-cert,c",
                                                                         po::bool_switch(&wantDefaultCert),
                                                                         "show default certificate, instead of "
                                                                         "identity")("identity,i",
                                                                                     po::value<Name>(&identityName),
                                                                                     "target identity")("key,K",
                                                                                                        po::value<Name>(
                                                                                                          &keyName),
                                                                                                        "target key")("quiet,q",
                                                                                                                      po::bool_switch(
    ...
    if (vm.count("key") > 0) {
        ...
        auto key = keyChain.getPib().getIdentity(identityName).getDefaultKey();
        if (wantDefaultKey) {
        ...
        auto identity = keyChain.getPib().getDefaultIdentity();
        if (wantDefaultKey) {
