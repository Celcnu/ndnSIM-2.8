...
const uint8_t GOOD_LINK[] = {0x06, 0xda, // Data
                             0x07, 0x14, // Name
                             ...
                             0x01, 0x19, 0x02,                                     // FreshnessPeriod
                             0x27, 0x10, 0x15, 0x1a,                               // Content
                             0x1f, 0x0c,                                           // LinkDelegation
                             0x1e, 0x01,                                           // LinkPreference
                             0x0a, 0x07, 0x07,                                     // Name
                             0x08, 0x05, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x1f, 0x0a, // LinkDelegation
                             0x1e, 0x01,                                           // LinkPreference
                             0x14, 0x07, 0x05,                                     // Name
                             0x08, 0x03, 0x6e, 0x64, 0x6e, 0x16, 0x1b,             // SignatureInfo
...
BOOST_AUTO_TEST_CASE(Decode)
{
    ...
    BOOST_CHECK_EQUAL(link.getDelegationList(), DelegationList({{10, Name("/local")}, {20, Name("/ndn")}}));
}
...
BOOST_AUTO_TEST_CASE(Encode)
{
    ...
    BOOST_CHECK_EQUAL(link2.getDelegationList(), DelegationList({{10, "/test1"}, {20, "/test2"}, {100, "/test3"}}));
}
...
{
    Link link1("/test", {{10, "/test1"}, {20, "/test2"}, {100, "/test3"}});
    BOOST_CHECK_EQUAL(link1.getDelegationList(), DelegationList({{10, "/test1"}, {20, "/test2"}, {100, "/test3"}}));
...
BOOST_AUTO_TEST_CASE(RemoveDelegation)
{
    ...
    BOOST_CHECK_EQUAL(link1.getDelegationList(), DelegationList({{10, "/test1"}, {100, "/test3"}}));

    signData(link1);
    Link link2(link1.wireEncode());
    BOOST_CHECK_EQUAL(link2.getDelegationList(), DelegationList({{10, "/test1"}, {100, "/test3"}}));
