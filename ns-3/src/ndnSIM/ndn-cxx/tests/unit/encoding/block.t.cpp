...
static const uint8_t TEST_BUFFER[] = {
  0x42, 0x01, 0xfa, 0x01, 0x01, 0xfb, 0xfe, 0xff, 0xff, 0xff, 0xff, 0x00, // bug #4726
};
...
BOOST_AUTO_TEST_CASE(FromEncodingBuffer)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(b.value_begin(), b.value_end(), VALUE, VALUE + sizeof(VALUE));
...
{
    const uint8_t BUFFER[] = {0x70, 0x00, 0x71, 0x03, 0x86, 0x11, 0x24, 0x72, 0x00};
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(b2.value_begin(), b2.value_end(), EXPECTED_VALUE2,
                                  EXPECTED_VALUE2 + sizeof(EXPECTED_VALUE2));
...
template <>
const std::vector<uint8_t> MalformedInput<struct TlvTypeZero>::INPUT{0x00, 0x00};
template <>
const std::vector<uint8_t> MalformedInput<struct TlvTypeTooLarge>::INPUT{0xff, 0x00, 0x00, 0x00, 0x01,
                                                                         0x00, 0x00, 0x00, 0x00, 0x00};
template <>
const std::vector<uint8_t> MalformedInput<struct BadTlvLength>::INPUT{0x01, 0xff, 0x42, 0x42};
template <>
const std::vector<uint8_t> MalformedInput<struct TruncatedTlvValue>::INPUT{0x01, 0x02, 0x03};

using MalformedInputs = boost::mpl::vector<MalformedInput<TlvTypeZero>, MalformedInput<TlvTypeTooLarge>,
                                           MalformedInput<BadTlvLength>, MalformedInput<TruncatedTlvValue>>;
...
BOOST_AUTO_TEST_CASE(Parse)
{
    ...
    const uint8_t MALFORMED[] = {// TLV-LENGTH of nested element is greater than TLV-LENGTH of enclosing element
...
BOOST_AUTO_TEST_CASE(Print)
{
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(b), "14[16]=FF7E4E6B3B21C902660F16ED589FCCCC");
    ...
    b.parse();
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(b), "264[8]={5[2]=CADD,89[2]=4E42}");
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(b), "264[8]={89[2]=4E42,16[2]=2386}");
}
