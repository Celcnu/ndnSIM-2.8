...
BOOST_AUTO_TEST_CASE(Basic)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(e.buf(), e.buf() + e.size(), expected1, expected1 + sizeof(expected1));

    const Encoder& constE = e;
    BOOST_CHECK_EQUAL_COLLECTIONS(constE.buf(), constE.buf() + constE.size(), expected1, expected1 + sizeof(expected1));

    uint8_t expected2[] = {'t', 'e', 's', 't', '2', 't', 'e', 's', 't', '1',
                           't', 'e', 's', 't', '1', 't', 'e', 's', 't', '2'};
    BOOST_CHECK_EQUAL_COLLECTIONS(e1.begin(), e1.end(), expected2, expected2 + sizeof(expected2));
    const Encoder& constE1 = e1;
    BOOST_CHECK_EQUAL_COLLECTIONS(constE1.begin(), constE1.end(), expected2, expected2 + sizeof(expected2));
