...
#define CHECK_COMP_ERR(expr, whatstring)                                                                               \
    BOOST_CHECK_EXCEPTION(expr, Component::Error, [](const auto& e) { return boost::contains(e.what(), whatstring); })
...
testSha256Component(uint32_t type, const std::string& uriPrefix)
{
    ...
    const std::string hexPctCanonical =
      "%28%BA%D4%B5%27%5B%D3%92%DB%B6p%C7%5C%F0%B6o%13%F7%94%2B%21%E8%0FU%C0%E8k7GS%A5H";
...
using ContainerTypes =
  boost::mpl::vector<std::vector<uint8_t>, std::list<uint8_t>, std::vector<int8_t>, std::list<int8_t>>;
...
using ConventionTests =
  boost::mpl::vector<NumberWithMarker, SegmentMarker, SegmentTyped, SegmentOffsetMarker, ByteOffsetTyped, VersionMarker,
                     VersionTyped, TimestampMarker, TimestampTyped, SequenceNumberMarker, SequenceNumberTyped>;
