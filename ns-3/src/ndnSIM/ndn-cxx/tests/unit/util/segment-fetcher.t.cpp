  ...
  public:
    ...
    onInterest(const Interest& interest)
    {
        if (interest.getName().get(-1).isSegment()) {
            ...
            auto data = makeDataSegment("/hello/world/version0", interest.getName().get(-1).toSegment(),
            ...
            if (segmentsToDropOrNack.size() > 0 && segmentsToDropOrNack.front() == 0) {
                segmentsToDropOrNack.pop();
                if (sendNackInsteadOfDropping) {
                    lp::Nack nack = makeNack(interest, nackReason);
                    face.receive(nack);
...
BOOST_AUTO_TEST_CASE(ExceedMaxTimeout)
{
    ...
    shared_ptr<SegmentFetcher> fetcher =
      SegmentFetcher::start(face, Interest("/hello/world"), acceptValidator, options);
...
BOOST_AUTO_TEST_CASE(BasicSingleSegment)
{
    ...
    size_t nAfterSegmentTimedOut = 0;
    shared_ptr<SegmentFetcher> fetcher = SegmentFetcher::start(face, Interest("/hello/world"), acceptValidator);
    connectSignals(fetcher);
...
BOOST_AUTO_TEST_CASE(ValidationFailure)
{
    ...
    validator.getPolicy().setResultCallback([](const Name& name) { return name.at(-1).toSegment() % 2 == 0; });
    shared_ptr<SegmentFetcher> fetcher = SegmentFetcher::start(face, Interest("/hello/world"), validator);
    connectSignals(fetcher);
    ...
    fetcher->afterSegmentReceived.connect([&nRecvSegments](const Data& receivedSegment) { ++nRecvSegments; });

    size_t nValidatedSegments = 0;
    fetcher->afterSegmentValidated.connect(
      [&nValidatedSegments](const Data& validatedSegment) { ++nValidatedSegments; });
...
BOOST_AUTO_TEST_CASE(OutOfScopeTimeout)
{
    ...
    weak_ptr<SegmentFetcher> weakFetcher;
    {
        auto fetcher = SegmentFetcher::start(face, Interest("/localhost/nfd/faces/list"), acceptValidator, options);
        weakFetcher = fetcher;
        connectSignals(fetcher);
        fetcher->afterSegmentReceived.connect(bind([&nAfterSegmentReceived] { ++nAfterSegmentReceived; }));
        fetcher->afterSegmentValidated.connect(bind([&nAfterSegmentValidated] { ++nAfterSegmentValidated; }));
