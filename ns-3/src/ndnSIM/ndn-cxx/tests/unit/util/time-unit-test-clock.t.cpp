...
BOOST_AUTO_TEST_CASE(SystemClock)
{
    ...
    time::system_clock::TimePoint referenceTime = time::fromUnixTimestamp(time::milliseconds(1390966967032LL));
    ...
    BOOST_CHECK_EQUAL(time::toString(referenceTime, "%Y -- %d -- %B", std::locale("C")), "2014 -- 29 -- January");
    ...
    BOOST_CHECK_EQUAL(time::fromString("2014-01-29 03:42:47"), time::fromUnixTimestamp(1390966967_s));
    ...
    BOOST_CHECK_EQUAL(time::fromString("2014 -- 29 -- January", "%Y -- %d -- %B", std::locale("C")),
                      time::fromUnixTimestamp(1390953600_s));
}

BOOST_AUTO_TEST_CASE(SteadyClock)
{
    BOOST_CHECK_EQUAL(time::steady_clock::now().time_since_epoch(), time::steady_clock::duration::zero());
