...
class NotificationSubscriberFixture : public IdentityManagementTimeFixture {
  public:
    ...
    {
        notificationConn =
          subscriber.onNotification.connect(bind(&NotificationSubscriberFixture::afterNotification, this, _1));
        nackConn = subscriber.onNack.connect(bind(&NotificationSubscriberFixture::afterNack, this, _1));
        subscriber.onTimeout.connect(bind(&NotificationSubscriberFixture::afterTimeout, this));
        subscriber.onDecodeError.connect(bind(&NotificationSubscriberFixture::afterDecodeError, this, _1));
    }
    ...
    hasInitialRequest() const
    {
        ...
        return interest.getName() == streamPrefix && interest.getCanBePrefix() && interest.getMustBeFresh()
    ...
    getRequestSeqNum() const
    {
        ...
        if (streamPrefix.isPrefixOf(name) && name.size() == streamPrefix.size() + 1
            && interest.getInterestLifetime() == subscriber.getInterestLifetime())
