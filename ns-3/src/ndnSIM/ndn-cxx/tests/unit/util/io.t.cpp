...
class DecodableTypeTpl {
  public:
    ...
    explicit DecodableTypeTpl(const Block& block)
    {
...
{
    this->writeFile<std::string>("CEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n"
                                 "AAAAAAAAAAAA\n");
...
{
    this->writeFile<std::string>("CEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n"
...
BOOST_AUTO_TEST_CASE(SaveNoEncoding)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(content.begin(), content.end(), expected, expected + sizeof(expected));
}
...
class IdCertFixture : public FileIoFixture, public IdentityManagementFixture {
};
