  ...
  protected:
    ...
    addTest(const std::string& request, bool shouldSucceed, const std::string& expectedUri)
    {
        ...
        uri.canonize(bind(&CanonizeFixture::onCanonizeSuccess, this, tc, _1),
                     bind(&CanonizeFixture::onCanonizeFailure, this, tc, _1), m_io, 10_s);
    }
...
BOOST_AUTO_TEST_CASE(CanonizeEmptyCallback)
{
    ...
    // unsupported scheme
    FaceUri("null://").canonize(FaceUri::CanonizeSuccessCallback(), FaceUri::CanonizeFailureCallback(), io, 1_ms);

    // cannot resolve
    FaceUri("udp://192.0.2.333")
      .canonize(FaceUri::CanonizeSuccessCallback(), FaceUri::CanonizeFailureCallback(), io, 1_ms);

    // already canonical
    FaceUri("udp4://192.0.2.1:6363")
      .canonize(FaceUri::CanonizeSuccessCallback(), FaceUri::CanonizeFailureCallback(), io, 1_ms);

    // need DNS resolution
    FaceUri("udp://192.0.2.1:6363")
      .canonize(FaceUri::CanonizeSuccessCallback(), FaceUri::CanonizeFailureCallback(), io, 1_ms);
