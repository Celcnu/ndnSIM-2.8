...
class DnsFixture {
  public:
    ...
    void
    onSuccess(const IpAddress& resolvedAddress, const IpAddress& expectedAddress, bool isValid,
...
BOOST_AUTO_TEST_CASE(AsynchronousV4)
{
    ...
    asyncResolve("192.0.2.1",
                 bind(&DnsFixture::onSuccess, this, _1, IpAddress(address_v4::from_string("192.0.2.1")), true, true),
                 bind(&DnsFixture::onFailure, this, false), m_ioService);
...
BOOST_AUTO_TEST_CASE(AsynchronousV6)
{
    ...
    asyncResolve("2001:db8:3f9:0:3025:ccc5:eeeb:86d3",
                 bind(&DnsFixture::onSuccess, this, _1,
                      IpAddress(address_v6::from_string("2001:db8:3f9:0:3025:ccc5:eeeb:86d3")), true, true),
                 bind(&DnsFixture::onFailure, this, false), m_ioService);
...
BOOST_AUTO_TEST_CASE(AsynchronousV4AndV6)
{
    ...
    asyncResolve("www.named-data.net", bind(&DnsFixture::onSuccess, this, _1, IpAddress(address_v4()), true, false),
                 bind(&DnsFixture::onFailure, this, false), m_ioService, Ipv4Only());

    asyncResolve("a.root-servers.net", bind(&DnsFixture::onSuccess, this, _1, IpAddress(address_v4()), true, false),
                 bind(&DnsFixture::onFailure, this, false), m_ioService, Ipv4Only()); // request IPv4 address

    asyncResolve("a.root-servers.net", bind(&DnsFixture::onSuccess, this, _1, IpAddress(address_v6()), true, false),
                 bind(&DnsFixture::onFailure, this, false), m_ioService, Ipv6Only()); // request IPv6 address

    asyncResolve("ipv6.google.com", // only IPv6 address should be available
                 bind(&DnsFixture::onSuccess, this, _1, IpAddress(address_v6()), true, false),
                 bind(&DnsFixture::onFailure, this, false), m_ioService, Ipv6Only());

    asyncResolve("ipv6.google.com", // only IPv6 address should be available
                 bind(&DnsFixture::onSuccess, this, _1, IpAddress(address_v6()), false, false),
                 bind(&DnsFixture::onFailure, this, true), m_ioService, Ipv4Only()); // should fail
