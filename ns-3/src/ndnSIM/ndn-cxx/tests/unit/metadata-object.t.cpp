...
class MetadataObjectFixture : public IdentityManagementFixture {
  public:
    MetadataObjectFixture()
      ...
      , versionedContentName(Name(baseContentName).appendVersion(342092199154ULL))
      , metadataFullName(Name(baseContentName).append(metadataComponent).appendVersion(metadataVerNo).appendSegment(0))
    {
    }
...
BOOST_AUTO_TEST_CASE(EncodeDecode)
{
    ...
    BOOST_CHECK(baseContentName.isPrefixOf(metadata2.makeData(metadataFullName.getPrefix(-2), m_keyChain).getName()));
}
...
{
    // valid name
    Name name = Name("/ndn/unit/test").append(metadataComponent).appendVersion().appendSegment(0);
