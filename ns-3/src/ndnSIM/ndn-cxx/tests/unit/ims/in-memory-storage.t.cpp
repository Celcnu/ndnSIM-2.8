...
using InMemoryStorages =
  boost::mpl::vector<InMemoryStoragePersistent, InMemoryStorageFifo, InMemoryStorageLfu, InMemoryStorageLru>;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(DigestCalculation, T, InMemoryStorages)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(digest1->begin(), digest1->end(), entry.getFullName()[-1].value_begin(),
...
using InMemoryStoragesLimited = boost::mpl::vector<InMemoryStorageFifo, InMemoryStorageLfu, InMemoryStorageLru>;
...
BOOST_AUTO_TEST_CASE(ExactName_CanBePrefix)
{
    ...
    startInterest("/A").setCanBePrefix(true);
    BOOST_CHECK_EQUAL(find(), 2);
}
...
BOOST_AUTO_TEST_CASE(PrefixName)
{
    ...
    startInterest("/B").setCanBePrefix(true);
    BOOST_CHECK_EQUAL(find(), 2);
}
...
BOOST_AUTO_TEST_CASE(MustBeFresh)
{
    ...
    insert(
      3, "/A/3", [](Data& data) { data.setFreshnessPeriod(1_s); }, 1_s);
    insert(
      4, "/A/4", [](Data& data) { data.setFreshnessPeriod(1_h); }, 1_h);

    // lookup at exact same moment as insertion is not tested because this won't happen in reality

    advanceClocks(500_ms); // @500ms
    startInterest("/A").setCanBePrefix(true).setMustBeFresh(true);
