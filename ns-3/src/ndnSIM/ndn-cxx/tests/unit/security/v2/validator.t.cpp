...
BOOST_AUTO_TEST_CASE(ExpiredCert)
{
    ...
    SignatureInfo info;
    info.setValidityPeriod(ValidityPeriod(time::system_clock::now() - 2_h, time::system_clock::now() - 1_h));
...
BOOST_AUTO_TEST_CASE(InfiniteCertChain)
{
    processInterest = [this](const Interest& interest) {
        ...
        certificateName.append("looper").appendVersion();
        ...
        info.setValidityPeriod(
          security::ValidityPeriod(time::system_clock::now() - 10_days, time::system_clock::now() + 10_days));
...
BOOST_AUTO_TEST_CASE(LoopedCertChain)
{
    ...
    auto makeCert = [this](Key& key, const Key& signer) {
        ...
        SignatureInfo info;
        info.setValidityPeriod({time::system_clock::now() - 100_days, time::system_clock::now() + 100_days});
