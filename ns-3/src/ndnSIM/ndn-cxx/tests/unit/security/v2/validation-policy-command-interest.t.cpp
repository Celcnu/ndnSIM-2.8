...
class ValidationPolicyCommandInterestFixture
  ...
  public:
    ...
    {
        return m_signer.makeCommandInterest(Name(identity.getName()).append("CMD"), signingByIdentity(identity));
    }
...
using ValidationPolicyAcceptAllCommands =
  ValidationPolicyCommandInterestFixture<DefaultOptions, ValidationPolicyAcceptAll>;
...
BOOST_AUTO_TEST_CASE(MissingKeyLocator)
{
    ...
    setNameComponent(i1, command_interest::POS_SIG_INFO, sigInfo.wireEncode().begin(), sigInfo.wireEncode().end());
...
typedef boost::mpl::vector<GracePeriod<boost::mpl::int_<0>>, GracePeriod<boost::mpl::int_<-1>>> GraceNonPositiveValues;
