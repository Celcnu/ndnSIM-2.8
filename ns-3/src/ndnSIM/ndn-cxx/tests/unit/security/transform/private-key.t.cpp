...
BOOST_AUTO_TEST_CASE(KeyDigest)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(digest->begin(), digest->end(), expected, expected + sizeof(expected));
}
...
struct EcKeyTestData {
    ...
    const std::string privateKeyPkcs1 = "MIIBaAIBAQQgRxwcbzK9RV6AHYFsDcykI86o3M/a1KlJn0z8PcLMBZOggfowgfcC\n"
...
checkPkcs8Encoding(ConstBufferPtr encoding, const std::string& password, ConstBufferPtr pkcs1)
{
    ...
    OBufferStream os;
    sKey.savePkcs1(os);
    BOOST_CHECK_EQUAL_COLLECTIONS(pkcs1->begin(), pkcs1->end(), os.buf()->begin(), os.buf()->end());
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(SaveLoad, T, KeyTestDataSets)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(sKeyPkcs1Base64, sKeyPkcs1Base64 + sKeyPkcs1Base64Len, os2.buf()->begin(),
                                  os2.buf()->end());

    // save key in pkcs1 format
    OBufferStream os3;
    BOOST_REQUIRE_NO_THROW(sKey.savePkcs1(os3));
    BOOST_CHECK_EQUAL_COLLECTIONS(sKeyPkcs1, sKeyPkcs1 + sKeyPkcs1Len, os3.buf()->begin(), os3.buf()->end());
    ...
    PrivateKey sKey5;
    BOOST_CHECK_NO_THROW(sKey5.loadPkcs8Base64(sKeyPkcs8Base64, sKeyPkcs8Base64Len, password.data(), password.size()));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(DerivePublicKey, T, KeyTestDataSets)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(pKeyBits->begin(), pKeyBits->end(), os.buf()->begin(), os.buf()->end());
}
...
BOOST_AUTO_TEST_CASE(RsaDecryption)
{
    ...
    const std::string cipherTextBase64 = "i2XNpZ2JbLa4JmBTdDrGmsd4/0C+p+BSCpW3MuPBNe5uChQ0eRO1dvjTnEqwSECY\n"
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(plainText, plainText + sizeof(plainText), decrypted->begin(), decrypted->end());
}
...
using KeyGenParams = boost::mpl::vector<RsaKeyGenParams, EcKeyGenParams, HmacKeyGenParams>;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(GenerateKey, T, KeyGenParams)
{
    ...
    OBufferStream os;
    BOOST_REQUIRE_NO_THROW(bufferSource(data, sizeof(data)) >> signerFilter(DigestAlgorithm::SHA256, *sKey)
                           >> streamSink(os));
    auto sig = os.buf();
