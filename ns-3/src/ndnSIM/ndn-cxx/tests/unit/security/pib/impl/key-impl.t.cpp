...
class OverwriteFixture : public ndn::security::tests::PibDataFixture, public ndn::tests::IdentityManagementFixture {
};
...
BOOST_FIXTURE_TEST_CASE(Overwrite, OverwriteFixture)
{
    ...
    SignatureInfo info;
    info.setValidityPeriod(ValidityPeriod(time::system_clock::now(), time::system_clock::now() + 1_s));
