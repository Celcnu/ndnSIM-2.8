...
using TestBackEnds = boost::mpl::vector<
  ...
  BackEndWrapperOsx,
#endif // NDN_CXX_HAVE_OSX_FRAMEWORKS
  BackEndWrapperMem, BackEndWrapperFile>;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(RsaDecryption, T, TestBackEnds)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(content, content + sizeof(content), plainText->begin(), plainText->end());
...
BOOST_AUTO_TEST_CASE(HmacSigningAndVerifying)
{
    ...
    bool result =
      key->verify(DigestAlgorithm::SHA256, content, sizeof(content), sigBlock.value(), sigBlock.value_size());
    BOOST_CHECK_EQUAL(result, true);
...
{
    const std::string privKeyPkcs1 = "MIIEpAIBAAKCAQEAw0WM1/WhAxyLtEqsiAJgWDZWuzkYpeYVdeeZcqRZzzfRgBQT\n"
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(privKeyPkcs1.begin(), privKeyPkcs1.end(), pkcs1->begin(), pkcs1->end());
