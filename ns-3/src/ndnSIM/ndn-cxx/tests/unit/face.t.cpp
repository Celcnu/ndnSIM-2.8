...
class FaceFixture : public IdentityManagementTimeFixture {
  public:
    ...
    bool
    runPrefixReg(
      function<void(const RegisterPrefixSuccessCallback& success, const RegisterPrefixFailureCallback& failure)> f)
    {
        boost::logic::tribool result = boost::logic::indeterminate;
        f([&](const Name&) { result = true; }, [&](const Name&, const std::string&) { result = false; });
    ...
    bool
    runPrefixUnreg(
      function<void(const UnregisterPrefixSuccessCallback& success, const UnregisterPrefixFailureCallback& failure)> f)
    {
        boost::logic::tribool result = boost::logic::indeterminate;
...
BOOST_AUTO_TEST_CASE(ExpressInterestData)
{
    ...
    face.expressInterest(
      *makeInterest("/Hello/World", true, 50_ms),
      [&](const Interest& i, const Data& d) {
          BOOST_CHECK(i.getName().isPrefixOf(d.getName()));
          BOOST_CHECK_EQUAL(i.getName(), "/Hello/World");
          ++nData;
      },
      bind([] { BOOST_FAIL("Unexpected Nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));
    ...
    size_t nTimeouts = 0;
    face.expressInterest(*makeInterest("/Hello/World/a/2", false, 50_ms), bind([] {}), bind([] {}),
...
BOOST_AUTO_TEST_CASE(ExpressMultipleInterestData)
{
    ...
    face.expressInterest(
      *makeInterest("/Hello/World", true, 50_ms), [&](const Interest& i, const Data& d) { ++nData; },
      bind([] { BOOST_FAIL("Unexpected Nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));

    face.expressInterest(
      *makeInterest("/Hello/World/a", true, 50_ms), [&](const Interest& i, const Data& d) { ++nData; },
      bind([] { BOOST_FAIL("Unexpected Nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));
...
{
    face.expressInterest(*makeInterest("/Hello/World", true), nullptr, bind([] { BOOST_FAIL("Unexpected Nack"); }),
...
{
    size_t nTimeouts = 0;
    face.expressInterest(*makeInterest("/Hello/World", false, 50_ms), bind([] { BOOST_FAIL("Unexpected Data"); }),
                         bind([] { BOOST_FAIL("Unexpected Nack"); }), [&nTimeouts](const Interest& i) {
                             BOOST_CHECK_EQUAL(i.getName(), "/Hello/World");
                             ++nTimeouts;
                         });
...
{
    face.expressInterest(*makeInterest("/Hello/World", false, 50_ms), bind([] { BOOST_FAIL("Unexpected Data"); }),
                         bind([] { BOOST_FAIL("Unexpected Nack"); }), nullptr);
    advanceClocks(40_ms);

    BOOST_CHECK_NO_THROW(do { advanceClocks(6_ms, 2); } while (false));
}
...
BOOST_AUTO_TEST_CASE(ExpressInterestNack)
{
    ...
    face.expressInterest(
      *interest, bind([] { BOOST_FAIL("Unexpected Data"); }),
...
BOOST_AUTO_TEST_CASE(ExpressMultipleInterestNack)
{
    ...
    face.expressInterest(
      *interest, bind([] { BOOST_FAIL("Unexpected Data"); }), [&](const Interest& i, const lp::Nack& n) { ++nNacks; },
      bind([] { BOOST_FAIL("Unexpected timeout"); }));
...
{
    face.expressInterest(*makeInterest("/Hello/World"), bind([] { BOOST_FAIL("Unexpected Data"); }), nullptr,
                         bind([] { BOOST_FAIL("Unexpected timeout"); }));
    advanceClocks(1_ms);
...
{
    auto hdl =
      face.expressInterest(*makeInterest("/Hello/World", true, 50_ms), bind([] { BOOST_FAIL("Unexpected data"); }),
                           bind([] { BOOST_FAIL("Unexpected nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));
    advanceClocks(10_ms);
...
{
    face.expressInterest(*makeInterest("/Hello/World/0", false, 50_ms), bind([] { BOOST_FAIL("Unexpected data"); }),
                         bind([] { BOOST_FAIL("Unexpected nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));

    face.expressInterest(*makeInterest("/Hello/World/1", false, 50_ms), bind([] { BOOST_FAIL("Unexpected data"); }),
                         bind([] { BOOST_FAIL("Unexpected nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));
...
{
    {
        ...
        face2.expressInterest(*makeInterest("/Hello/World", false, 50_ms), nullptr, nullptr, nullptr);
        advanceClocks(50_ms, 2);
    }
...
{
    face.expressInterest(
      *makeInterest("/localhost/notification/1"),
      [&](const Interest& i, const Data& d) { face.put(*makeData("/chronosync/sampleDigest/1")); }, nullptr, nullptr);
    advanceClocks(10_ms);
...
BOOST_AUTO_TEST_CASE(PutDataLoopback)
{
    ...
    face.expressInterest(*makeInterest("/A", true), bind([&] { hasData = true; }),
                         bind([] { BOOST_FAIL("Unexpected nack"); }), bind([] { BOOST_FAIL("Unexpected timeout"); }));
    advanceClocks(1_ms);
...
BOOST_AUTO_TEST_CASE(PutMultipleData)
{
    ...
    face.setInterestFilter("/", bind([&] {
                               hasInterest1 = true;
                               // sending Data right away from the first destination, don't care whether Interest goes
...
BOOST_AUTO_TEST_CASE(PutNack)
{
    ...
    face.put(
      makeNack(*makeInterest("/unsolicited", false, DEFAULT_INTEREST_LIFETIME, 18645250), lp::NackReason::NO_ROUTE));
    advanceClocks(10_ms);
...
BOOST_AUTO_TEST_CASE(SetUnsetInterestFilter)
{
    ...
    size_t nRegs = 0;
    auto hdl =
      face.setInterestFilter("/Hello/World", bind([&nInterests] { ++nInterests; }), bind([&nRegs] { ++nRegs; }),
...
{
    size_t nInterests = 0;
    auto hdl = face.setInterestFilter("/Hello/World", bind([&nInterests] { ++nInterests; }),
                                      bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));
    advanceClocks(25_ms, 4);
    BOOST_CHECK_EQUAL(nInterests, 0);
...
BOOST_FIXTURE_TEST_CASE(SetInterestFilterFail, FaceFixture<NoPrefixRegReply>)
{
    ...
    size_t nRegFailed = 0;
    face.setInterestFilter("/Hello/World", bind([] { BOOST_FAIL("Unexpected Interest"); }),
...
BOOST_AUTO_TEST_CASE(RegisterUnregisterPrefixHandle)
{
    ...
    BOOST_CHECK(runPrefixReg(
      [&](const auto& success, const auto& failure) { hdl = face.registerPrefix("/Hello/World", success, failure); }));

    BOOST_CHECK(runPrefixUnreg([&](const auto& success, const auto& failure) { hdl.unregister(success, failure); }));
}

BOOST_AUTO_TEST_CASE(SimilarFilters)
{
    size_t nInInterests1 = 0;
    face.setInterestFilter("/Hello/World", bind([&nInInterests1] { ++nInInterests1; }), nullptr,
                           bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));

    size_t nInInterests2 = 0;
    face.setInterestFilter("/Hello", bind([&nInInterests2] { ++nInInterests2; }), nullptr,
                           bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));

    size_t nInInterests3 = 0;
    face.setInterestFilter("/Los/Angeles/Lakers", bind([&nInInterests3] { ++nInInterests3; }), nullptr,
                           bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));
...
BOOST_AUTO_TEST_CASE(SetRegexFilterError)
{
    face.setInterestFilter(
      InterestFilter("/Hello/World", "<><b><c>?"),
      [](const Name&, const Interest&) { BOOST_FAIL("InterestFilter::Error should have been triggered"); }, nullptr,
      bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));
...
BOOST_AUTO_TEST_CASE(SetRegexFilter)
{
    ...
    face.setInterestFilter(InterestFilter("/Hello/World", "<><b><c>?"), bind([&nInInterests] { ++nInInterests; }),
                           nullptr, bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));
...
BOOST_AUTO_TEST_CASE(SetRegexFilterAndRegister)
{
    ...
    face.registerPrefix("/Hello/World", bind([&nRegSuccesses] { ++nRegSuccesses; }),
                        bind([] { BOOST_FAIL("Unexpected setInterestFilter failure"); }));
