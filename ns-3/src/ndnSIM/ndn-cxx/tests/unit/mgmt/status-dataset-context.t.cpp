...
class StatusDatasetContextFixture {
  ...
  protected:
    StatusDatasetContextFixture()
      ...
      , context(
          *interest,
          [this](const Name& dataName, const Block& content, time::milliseconds imsFresh, bool isFinalBlock) {
              SendDataArgs args{dataName, content, imsFresh, isFinalBlock};
              sendDataHistory.push_back(args);
          },
          [this](const ControlResponse& resp) { sendNackHistory.push_back(resp); })
