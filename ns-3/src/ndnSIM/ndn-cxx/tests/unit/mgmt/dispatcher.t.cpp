...
class VoidParameters : public mgmt::ControlParameters {
  public:
    explicit VoidParameters(const Block& wire)
    {
        wireDecode(wire);
    }
...
{
    return [](const Name& prefix, const Interest& interest, const ControlParameters* params, AcceptContinuation accept,
...
{
    BOOST_CHECK_NO_THROW(dispatcher.addControlCommand<VoidParameters>("test/1", makeAcceptAllAuthorization(),
                                                                      bind([] { return true; }), bind([] {})));
    BOOST_CHECK_NO_THROW(dispatcher.addControlCommand<VoidParameters>("test/2", makeAcceptAllAuthorization(),
                                                                      bind([] { return true; }), bind([] {})));

    BOOST_CHECK_THROW(dispatcher.addControlCommand<VoidParameters>("test", makeAcceptAllAuthorization(),
                                                                   bind([] { return true; }), bind([] {})),
                      std::out_of_range);

    BOOST_CHECK_NO_THROW(dispatcher.addStatusDataset("status/1", makeAcceptAllAuthorization(), bind([] {})));
    BOOST_CHECK_NO_THROW(dispatcher.addStatusDataset("status/2", makeAcceptAllAuthorization(), bind([] {})));
    BOOST_CHECK_THROW(dispatcher.addStatusDataset("status", makeAcceptAllAuthorization(), bind([] {})),
                      std::out_of_range);
    ...
    BOOST_CHECK_THROW(dispatcher.addControlCommand<VoidParameters>("test/3", makeAcceptAllAuthorization(),
                                                                   bind([] { return true; }), bind([] {})),
                      std::domain_error);

    BOOST_CHECK_THROW(dispatcher.addStatusDataset("status/3", makeAcceptAllAuthorization(), bind([] {})),
                      std::domain_error);
...
BOOST_AUTO_TEST_CASE(AddRemoveTopPrefix)
{
    ...
    dispatcher.addControlCommand<VoidParameters>("test/1", makeAcceptAllAuthorization(), bind([] { return true; }),
                                                 bind([&nCallbackCalled] { ++nCallbackCalled["test/1"]; }));

    dispatcher.addControlCommand<VoidParameters>("test/2", makeAcceptAllAuthorization(), bind([] { return true; }),
...
BOOST_AUTO_TEST_CASE(ControlCommand)
{
    ...
    dispatcher.addControlCommand<VoidParameters>("test", makeTestAuthorization(), bind([] { return true; }),
...
class StatefulParameters : public mgmt::ControlParameters {
  public:
    explicit StatefulParameters(const Block& wire)
    {
        wireDecode(wire);
    }
...
BOOST_AUTO_TEST_CASE(ControlCommandAsyncAuthorization) // Bug 4059
{
    ...
    auto authorization = [&authorizationAccept](const Name& prefix, const Interest& interest,
                                                const ControlParameters* params, AcceptContinuation accept,
                                                RejectContinuation reject) { authorizationAccept = accept; };
...
BOOST_AUTO_TEST_CASE(StatusDataset)
{
    ...
    dispatcher.addStatusDataset("test/small", makeTestAuthorization(),
    ...
    dispatcher.addStatusDataset("test/large", makeTestAuthorization(),
    ...
    dispatcher.addStatusDataset("test/reject", makeTestAuthorization(),
                                [](const Name& prefix, const Interest& interest, StatusDatasetContext& context) {
                                    context.reject();
                                });
    ...
    content = [&dataInStorage]() -> Block {
        EncodingBuffer encoder;
        size_t valueLength =
          encoder.prependByteArray(dataInStorage[1].getContent().value(), dataInStorage[1].getContent().value_size());
        valueLength +=
          encoder.prependByteArray(dataInStorage[0].getContent().value(), dataInStorage[0].getContent().value_size());
