...
makeRoute()
{
    return Route().setFaceId(1).setOrigin(ROUTE_ORIGIN_NLSR).setCost(100).setFlags(ROUTE_FLAG_CAPTURE);
}

static RibEntry
makeRibEntry()
{
    return RibEntry().setName("/hello/world").addRoute(makeRoute().setExpirationPeriod(10_s));
}
...
BOOST_AUTO_TEST_CASE(RouteEncode)
{
    ...
    static const uint8_t expected[] = {0x81, 0x10, 0x69, 0x01, 0x01, 0x6f, 0x01, 0x80, 0x6a,
                                       0x01, 0x64, 0x6c, 0x01, 0x02, 0x6d, 0x02, 0x27, 0x10};
...
BOOST_AUTO_TEST_CASE(RouteNoExpirationPeriodEncode)
{
    ...
    static const uint8_t expected[] = {0x81, 0x0C, 0x69, 0x01, 0x01, 0x6f, 0x01,
                                       0x80, 0x6a, 0x01, 0x64, 0x6c, 0x01, 0x02};
    BOOST_CHECK_EQUAL_COLLECTIONS(expected, expected + sizeof(expected), wire.begin(), wire.end());
...
BOOST_AUTO_TEST_CASE(RibEntryEncode)
{
    ...
    static const uint8_t expected[] = {0x80, 0x34, 0x07, 0x0e, 0x08, 0x05, 0x68, 0x65, 0x6c, 0x6c, 0x6f,
                                       0x08, 0x05, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x81, 0x10, 0x69, 0x01,
                                       0x01, 0x6f, 0x01, 0x80, 0x6a, 0x01, 0x64, 0x6c, 0x01, 0x02, 0x6d,
                                       0x02, 0x27, 0x10, 0x81, 0x10, 0x69, 0x01, 0x02, 0x6f, 0x01, 0x00,
                                       0x6a, 0x01, 0x20, 0x6c, 0x01, 0x01, 0x6d, 0x02, 0x13, 0x88};
    BOOST_CHECK_EQUAL_COLLECTIONS(expected, expected + sizeof(expected), wire.begin(), wire.end());
...
BOOST_AUTO_TEST_CASE(RibEntryEquality)
{
    ...
    entry2 = entry1;
    auto r1 = Route().setFaceId(1).setCost(1000);
    entry1.addRoute(r1);
    BOOST_CHECK_NE(entry1, entry2);
    BOOST_CHECK_NE(entry2, entry1);

    auto r42 = Route().setFaceId(42).setCost(42);
    entry1.addRoute(r42);
    entry2.addRoute(r42).addRoute(r1);
    BOOST_CHECK_EQUAL(entry1, entry2); // order of Routes is irrelevant
    BOOST_CHECK_EQUAL(entry2, entry1);

    entry1 = entry2 = makeRibEntry();
    entry1.addRoute(r1).addRoute(r42);
    entry2.addRoute(r42).addRoute(r42);
...
BOOST_AUTO_TEST_CASE(Print)
{
    ...
    RibEntry entry;
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(entry), "RibEntry(Prefix: /,\n"
                                                               "         Routes: []\n"
                                                               "         )");

    entry = makeRibEntry();
    entry.addRoute(Route().setFaceId(2).setOrigin(ROUTE_ORIGIN_STATIC).setCost(32).setFlags(ROUTE_FLAG_CHILD_INHERIT));
