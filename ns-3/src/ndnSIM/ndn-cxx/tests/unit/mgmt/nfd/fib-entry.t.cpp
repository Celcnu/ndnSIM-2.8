...
makeFibEntry()
{
    ...
    for (size_t i = 1; i < 4; i++) {
        nexthops.push_back(NextHopRecord().setFaceId(i * 10).setCost(i * 100 + 100));
    }

    return FibEntry().setPrefix("/this/is/a/test").setNextHopRecords(nexthops.begin(), nexthops.end());
}

BOOST_AUTO_TEST_CASE(NextHopRecordEncode)
{
    NextHopRecord record1;
    record1.setFaceId(10).setCost(200);
...
BOOST_AUTO_TEST_CASE(FibEntryNoNextHopsEncode)
{
    ...
    static const uint8_t expected[] = {0x80, 0x15, 0x07, 0x13, 0x08, 0x04, 0x74, 0x68, 0x69, 0x73, 0x08, 0x02,
                                       0x69, 0x73, 0x08, 0x01, 0x61, 0x08, 0x04, 0x74, 0x65, 0x73, 0x74};
    BOOST_CHECK_EQUAL_COLLECTIONS(expected, expected + sizeof(expected), wire.begin(), wire.end());
...
BOOST_AUTO_TEST_CASE(FibEntryEquality)
{
    ...
    entry2 = entry1;
    auto nh1 = NextHopRecord().setFaceId(1).setCost(1000);
    entry1.addNextHopRecord(nh1);
    BOOST_CHECK_NE(entry1, entry2);
    BOOST_CHECK_NE(entry2, entry1);

    auto nh42 = NextHopRecord().setFaceId(42).setCost(42);
    entry1.addNextHopRecord(nh42);
    entry2.addNextHopRecord(nh42).addNextHopRecord(nh1);
    BOOST_CHECK_EQUAL(entry1, entry2); // order of NextHopRecords is irrelevant
    BOOST_CHECK_EQUAL(entry2, entry1);

    entry1 = entry2 = makeFibEntry();
    entry1.addNextHopRecord(nh1).addNextHopRecord(nh42);
    entry2.addNextHopRecord(nh42).addNextHopRecord(nh42);
...
BOOST_AUTO_TEST_CASE(Print)
{
    ...
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(record), "NextHopRecord(FaceId: 0, Cost: 0)");

    FibEntry entry;
    BOOST_CHECK_EQUAL(boost::lexical_cast<std::string>(entry), "FibEntry(Prefix: /,\n"
                                                               "         NextHops: []\n"
                                                               "         )");
