...
BOOST_AUTO_TEST_CASE(Timeout)
{
    ...
    controller
      .fetch<FaceDataset>([](const std::vector<FaceStatus>& result) { BOOST_FAIL("fetchDataset should not succeed"); },
                          datasetFailCallback, options);
...
{
    controller.fetch<FaceDataset>(nullptr, datasetFailCallback);
    this->advanceClocks(500_ms);
    BOOST_CHECK_EQUAL(controller.m_fetchers.size(), 1);
