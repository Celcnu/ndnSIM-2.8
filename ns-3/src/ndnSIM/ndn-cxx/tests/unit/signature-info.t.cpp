...
const uint8_t sigInfoRsa[] = {0x16, 0x1b, // SignatureInfo
...
BOOST_AUTO_TEST_CASE(Constructor)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(sigInfoBlock.wire(), sigInfoBlock.wire() + sigInfoBlock.size(), encoded.wire(),
...
{
    const uint8_t error1[] = {0x15, 0x1b, // Wrong SignatureInfo (0x16, 0x1b)
                              0x1b, 0x01, // SignatureType
                              0x01,       // Sha256WithRsa
                              0x1c, 0x16, // KeyLocator
                              0x07, 0x14, // Name
    ...
    const uint8_t error3[] = {0x16, 0x01, // SignatureInfo
...
{
    const uint8_t sigInfo[] = {0x16, 0x45, // SignatureInfo
                               0x1b, 0x01, // SignatureType
                               0x01,       // Sha256WithRsa
                               0x1c, 0x16, // KeyLocator
                               0x07, 0x14, // Name
                               0x08, 0x04, 0x74, 0x65, 0x73, 0x74, 0x08, 0x03, 0x6b, 0x65, 0x79, 0x08,
                               0x07, 0x6c, 0x6f, 0x63, 0x61, 0x74, 0x6f, 0x72, 0xfd, 0x00, 0xfd, 0x26, // ValidityPeriod
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(sigInfo, sigInfo + sizeof(sigInfo), encoded.wire(), encoded.wire() + encoded.size());
...
BOOST_AUTO_TEST_CASE(OtherTlvsEncoding) // Bug #3914
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(infoBytes, infoBytes + sizeof(infoBytes), info1.wireEncode().begin(),
                                  info1.wireEncode().end());
}
