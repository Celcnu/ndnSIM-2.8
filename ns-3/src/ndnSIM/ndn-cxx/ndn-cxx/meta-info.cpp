...
static_assert(std::is_base_of<tlv::Error, MetaInfo::Error>::value, "MetaInfo::Error must inherit from tlv::Error");
...
{
    auto it =
      std::find_if(m_appMetaInfo.begin(), m_appMetaInfo.end(), [=](const Block& b) { return b.type() == tlvType; });
