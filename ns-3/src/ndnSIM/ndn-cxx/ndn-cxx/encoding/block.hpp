...
class Block {
  ...
  public: // construction, assignment
     ...
     */
    Block(ConstBufferPtr buffer, uint32_t type, Buffer::const_iterator begin, Buffer::const_iterator end,
     ...
     */
    NDN_CXX_NODISCARD static std::tuple<bool, Block> fromBuffer(ConstBufferPtr buffer, size_t offset);
     ...
     *  @return `true` and the parsed Block if parsing succeeds; otherwise `false` and an invalid Block
     */
    NDN_CXX_NODISCARD static std::tuple<bool, Block> fromBuffer(const uint8_t* buf, size_t bufSize);
