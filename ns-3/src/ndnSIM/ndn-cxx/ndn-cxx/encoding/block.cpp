...
Block::Block(ConstBufferPtr buffer, Buffer::const_iterator begin, Buffer::const_iterator end, bool verifyLength)
  ...
  , m_size(m_end - m_begin)
{
    ...
    if (&*begin < bufferBegin || &*begin > bufferEnd || &*end < bufferBegin || &*end > bufferEnd) {
...
Block::Block(const Block& block, Buffer::const_iterator begin, Buffer::const_iterator end, bool verifyLength)
  : Block(block.m_buffer, begin, end, verifyLength)
{
}

Block::Block(ConstBufferPtr buffer, uint32_t type, Buffer::const_iterator begin, Buffer::const_iterator end,
...
Block::fromBuffer(const uint8_t* buf, size_t bufSize)
{
    ...
    return std::make_tuple(true, Block(b, type, b->begin(), b->end(), b->begin() + typeLengthSize, b->end()));
}
...
Block::get(uint32_t type) const
{
    ...
    NDN_THROW(Error("No sub-element of type " + to_string(type) + " found in block of type " + to_string(m_type)));
}
...
{
    return lhs.type() == rhs.type() && lhs.value_size() == rhs.value_size()
           && (lhs.value_size() == 0 || std::memcmp(lhs.value(), rhs.value(), lhs.value_size()) == 0);
}
