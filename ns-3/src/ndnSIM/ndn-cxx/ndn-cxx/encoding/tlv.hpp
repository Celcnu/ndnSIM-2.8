...
[[deprecated("use GenericNameComponent")]] constexpr int NameComponent = GenericNameComponent;
 ...
 */
template <typename Iterator, typename DecayedIterator = std::decay_t<Iterator>,
...
template <typename Iterator>
class ReadNumber : public std::conditional_t<shouldSelectContiguousReadNumber<Iterator>(), ReadNumberFast<Iterator>,
                                             ReadNumberSlow<Iterator>> {
};
...
sizeOfVarNumber(uint64_t number) noexcept
{
    return number < 253                                     ? 1
           ...
           : number <= std::numeric_limits<uint32_t>::max() ? 5
                                                            : 9;
}
