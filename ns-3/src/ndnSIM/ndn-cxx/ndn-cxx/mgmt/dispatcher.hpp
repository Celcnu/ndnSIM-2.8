 ...
 */
typedef std::function<void(const Name& prefix, const Interest& interest, const ControlParameters* params,
                           const AcceptContinuation& accept, const RejectContinuation& reject)>
  Authorization;
 ...
 */
typedef std::function<void(const Name& prefix, const Interest& interest, StatusDatasetContext& context)>
...
class Dispatcher : noncopyable {
  public:
     ...
     */
    Dispatcher(Face& face, KeyChain& keyChain, const security::SigningInfo& signingInfo = security::SigningInfo(),
     ...
     *     prefix
     */
    void addTopPrefix(const Name& prefix, bool wantRegister = true,
  ...
  public: // ControlCommand
    ...
    void addControlCommand(const PartialName& relPrefix, Authorization authorize, ValidateParameters validate,
  ...
  private:
    typedef std::function<void(const Name& prefix, const Interest& interest)> InterestHandler;

    typedef std::function<void(const std::string& requester, const Name& prefix, const Interest& interest,
                               const shared_ptr<ControlParameters>&)>
      AuthorizationAcceptedCallback;

    typedef std::function<void(RejectReply act, const Interest& interest)> AuthorizationRejectedCallback;
    ...
    bool isOverlappedWithOthers(const PartialName& relPrefix) const;
     ...
     */
    void afterAuthorizationRejected(RejectReply act, const Interest& interest);
     ...
     */
    void queryStorage(const Name& prefix, const Interest& interest, const InterestHandler& missContinuation);

    enum class SendDestination { NONE = 0, FACE = 1, IMS = 2, FACE_AND_IMS = 3 };
     ...
     */
    void sendData(const Name& dataName, const Block& content, const MetaInfo& metaInfo, SendDestination destination,
     ...
     */
    void sendOnFace(const Data& data);
     ...
     */
    void processControlCommandInterest(const Name& prefix, const Name& relPrefix, const Interest& interest,
                                       const ControlParametersParser& parser, const Authorization& authorization,
     ...
     */
    void
    processAuthorizedControlCommandInterest(const std::string& requester, const Name& prefix, const Interest& interest,
                                            const shared_ptr<ControlParameters>& parameters,
                                            const ValidateParameters& validate, const ControlCommandHandler& handler);

    void sendControlResponse(const ControlResponse& resp, const Interest& interest, bool isNack = false);
     ...
     */
    void processStatusDatasetInterest(const Name& prefix, const Interest& interest, const Authorization& authorization,
                                      const AuthorizationAcceptedCallback& accepted,
                                      const AuthorizationRejectedCallback& rejected);
     ...
     */
    void processAuthorizedStatusDatasetInterest(const std::string& requester, const Name& prefix,
                                                const Interest& interest, const StatusDatasetHandler& handler);
     ...
     */
    void sendStatusDatasetSegment(const Name& dataName, const Block& content, time::milliseconds imsFresh,
                                  bool isFinalBlock);

    void postNotification(const Block& notification, const PartialName& relPrefix);
  ...
  private:
    ...
    NDN_CXX_PUBLIC_WITH_TESTS_ELSE_PRIVATE : InMemoryStorageFifo m_storage;
};

template <typename CP>
void
Dispatcher::addControlCommand(const PartialName& relPrefix, Authorization authorize, ValidateParameters validate,
                              ControlCommandHandler handle)
{
    ...
    AuthorizationAcceptedCallback accepted = bind(&Dispatcher::processAuthorizedControlCommandInterest, this, _1, _2,
                                                  _3, _4, std::move(validate), std::move(handle));

    AuthorizationRejectedCallback rejected = bind(&Dispatcher::afterAuthorizationRejected, this, _1, _2);

    m_handlers[relPrefix] = bind(&Dispatcher::processControlCommandInterest, this, _1, relPrefix, _2, std::move(parser),
