...
{
    return a.getFaceId() == b.getFaceId() && a.getOrigin() == b.getOrigin() && a.getCost() == b.getCost()
           && a.getFlags() == b.getFlags() && a.getExpirationPeriod() == b.getExpirationPeriod();
}
...
operator==(const RibEntry& a, const RibEntry& b)
{
    ...
    if (a.getName() != b.getName() || aRoutes.size() != bRoutes.size())
        return false;

    std::vector<bool> matched(bRoutes.size(), false);
    return std::all_of(aRoutes.begin(), aRoutes.end(), [&](const Route& route) {
...
operator<<(std::ostream& os, const RibEntry& entry)
{
    ...
    std::copy(entry.getRoutes().begin(), entry.getRoutes().end(), make_ostream_joiner(os, ",\n                  "));
