...
void
Controller::startCommand(const shared_ptr<ControlCommand>& command, const ControlParameters& parameters,
                         ...
                         const CommandOptions& options)
{
    ...
    m_face.expressInterest(
      interest, [=](const Interest&, const Data& data) { processCommandResponse(data, command, onSuccess, onFailure); },
...
void
Controller::processCommandResponse(const Data& data, const shared_ptr<ControlCommand>& command,
                                   const CommandSucceedCallback& onSuccess, const CommandFailCallback& onFailure)
{
    m_validator.validate(
      data, [=](const Data& data) { processValidatedCommandResponse(data, command, onSuccess, onFailure); },
...
void
Controller::processValidatedCommandResponse(const Data& data, const shared_ptr<ControlCommand>& command,
...
void
Controller::fetchDataset(const Name& prefix, const std::function<void(ConstBufferPtr)>& processResponse,
                         const DatasetFailCallback& onFailure, const CommandOptions& options)
{
    ...
    if (onFailure) {
        fetcher->onError.connect(
          [=](uint32_t code, const std::string& msg) { processDatasetFetchError(onFailure, code, msg); });
    }
...
void
Controller::processDatasetFetchError(const DatasetFailCallback& onFailure, uint32_t code, std::string msg)
{
