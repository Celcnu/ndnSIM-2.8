  ...
  : m_data(std::move(data))
{
    if (m_data->getContentType() != tlv::ContentType_PrefixAnn) {
        NDN_THROW(Error("Data is not a prefix announcement: ContentType is " + to_string(m_data->getContentType())));
    }

    const Name& dataName = m_data->getName();
    if (dataName.size() < 3 || dataName[-3] != KEYWORD_PA_COMP || !dataName[-2].isVersion()
        || !dataName[-1].isSegment()) {
...
const Data&
PrefixAnnouncement::toData(KeyChain& keyChain, const ndn::security::SigningInfo& si, optional<uint64_t> version) const
{
    if (!m_data) {
        ...
        content.push_back(makeNonNegativeIntegerBlock(tlv::nfd::ExpirationPeriod, m_expiration.count()));
        if (m_validity) {
...
{
    return lhs.getAnnouncedName() == rhs.getAnnouncedName() && lhs.getExpiration() == rhs.getExpiration()
