...
DelegationList::wireEncode(EncodingImpl<TAG>& encoder, uint32_t type) const
{
    if (!isValidTlvType(type)) {
        NDN_THROW(std::invalid_argument("Unexpected TLV-TYPE " + to_string(type) + " while encoding DelegationList"));
    }
...
bool
DelegationList::insert(uint64_t preference, const Name& name, InsertConflictResolution onConflict)
{
    switch (onConflict) {
        case INS_REPLACE:
...
DelegationList::eraseImpl(optional<uint64_t> preference, const Name& name)
{
    ...
    for (auto i = m_dels.begin(); i != m_dels.end();) {
        if ((!preference || i->preference == *preference) && i->name == name) {
            ++nErased;
            i = m_dels.erase(i);
        }
        else {
