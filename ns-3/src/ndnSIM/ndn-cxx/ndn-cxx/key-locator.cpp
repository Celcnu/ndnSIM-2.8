...
static_assert(std::is_base_of<tlv::Error, KeyLocator::Error>::value, "KeyLocator::Error must inherit from tlv::Error");
...
KeyLocator::wireEncode(EncodingImpl<TAG>& encoder) const
{
    ...
    auto visitor = overload([](monostate) {}, // nothing to encode, TLV-VALUE is empty
...
{
    auto visitor =
      overload([&](monostate) { os << "None"; }, [&](const Name& name) { os << "Name=" << name; },
               [&](const Block& digest) {
                   os << "KeyDigest=";
