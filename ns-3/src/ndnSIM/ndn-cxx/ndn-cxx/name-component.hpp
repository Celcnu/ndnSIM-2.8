...
class Component : public Block {
  ...
  public: // constructors
     ...
     */
    explicit Component(uint32_t type = tlv::GenericNameComponent);
     ...
     * Therefore, the caller must not change the underlying buffer.
     */
    explicit Component(ConstBufferPtr buffer)
     ...
     */
    explicit Component(const Buffer& buffer)
     ...
     */
    explicit Component(const char* str);
     ...
     * Bytes from the string are copied as is, and not interpreted as URI component.
     */
    explicit Component(const std::string& str);
