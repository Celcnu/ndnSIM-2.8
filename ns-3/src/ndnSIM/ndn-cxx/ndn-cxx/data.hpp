...
namespace ndn {
    ...
    class Data : public PacketBase, public std::enable_shared_from_this<Data> {
      public:
         ...
         *           using `make_shared`. Otherwise, `shared_from_this()` will trigger undefined
         * behavior.
         *
         * make_shared???
         ...
         *         SignatureInfo to @p encoder, but omit SignatureValue and outmost Type-Length of
         * 		   Data element. This is intended to be used with wireEncode(encoder, signatureValue).
         ...
         *  cached wire encoding (\c hasWire() is true), the cached encoding is returned and it
         * might be in v0.3 format.
         */
      ...
      public: // Data fields 数据的Get和Set
        ...
        /** @brief Get Content
         *
         *  The Content value is accessible through value()/value_size() or
      ...
      protected:
         ...
         */
        void resetWire(); // 每次set之后都要重置wire
      ...
      private:
        ...
        mutable Name m_fullName; ///< cached FullName computed from m_wire, 和m_name区别
    };

#ifndef DOXYGEN
    extern template size_t Data::wireEncode<encoding::EncoderTag>(EncodingBuffer&, bool) const;

    extern template size_t Data::wireEncode<encoding::EstimatorTag>(EncodingEstimator&, bool) const;
#endif
