...
Name
BackEnd::constructAsymmetricKeyName(const KeyHandle& keyHandle, const Name& identity, const KeyParams& params) const
{
    switch (params.getKeyIdType()) {
        case KeyIdType::USER_SPECIFIED: {
        ...
        case KeyIdType::SHA256: {
            ...
            OBufferStream os;
            bufferSource(*keyHandle.derivePublicKey()) >> digestFilter(DigestAlgorithm::SHA256) >> streamSink(os);
...
Name
BackEnd::constructHmacKeyName(const transform::PrivateKey& key, const Name& identity, const KeyParams& params) const
{
