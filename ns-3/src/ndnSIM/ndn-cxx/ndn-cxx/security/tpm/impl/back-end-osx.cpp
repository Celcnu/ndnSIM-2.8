...
exportItem(const KeyRefOsx& keyRef, transform::PrivateKey& outKey)
{
    ...
    outKey.loadPkcs8(CFDataGetBytePtr(exportedKey.get()), CFDataGetLength(exportedKey.get()), pw, std::strlen(pw));
}
...
void
BackEndOsx::doImportKey(const Name& keyName, const uint8_t* buf, size_t size, const char* pw, size_t pwLen)
{
