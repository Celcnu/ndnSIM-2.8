...
ValidationPolicyCommandInterest::cleanup()
{
    ...
    while ((!m_queue.empty() && m_queue.front().lastRefreshed <= expiring)
           || (m_options.maxRecords >= 0 && m_queue.size() > static_cast<size_t>(m_options.maxRecords))) {
        m_queue.pop_front();
    }
}
                                                      ...
                                                      const shared_ptr<ValidationState>& state) const
{
    ...
    if (name.size() < command_interest::MIN_SIZE) {
        state->fail(
          {ValidationError::POLICY_ERROR, "Command interest name `" + interest.getName().toUri() + "` is too short"});
    ...
    if (!timestampComp.isNumber()) {
        state->fail({ValidationError::POLICY_ERROR,
                     "Command interest `" + interest.getName().toUri() + "` doesn't include timestamp component"});
        return std::make_tuple(false, Name(), 0);
    }
...
bool
ValidationPolicyCommandInterest::checkTimestamp(const shared_ptr<ValidationState>& state, const Name& keyName,
                                                uint64_t timestamp)
{
    this->cleanup();
    ...
    if (timestampPoint < now - m_options.gracePeriod || timestampPoint > now + m_options.gracePeriod) {
        state->fail(
          {ValidationError::POLICY_ERROR, "Timestamp is outside the grace period for key " + keyName.toUri()});
        return false;
    }
