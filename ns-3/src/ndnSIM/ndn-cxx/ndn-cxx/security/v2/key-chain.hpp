...
class KeyChain : noncopyable {
  ...
  public: // Key management
     ...
     */
    Name
    createHmacKey(const Name& prefix = SigningInfo::getHmacIdentity(), const HmacKeyParams& params = HmacKeyParams());
  ...
  private:
    ...
    static PibFactories& getPibFactories();

    static TpmFactories& getTpmFactories();
    ...
    static const std::string& getDefaultPibScheme();

    static const std::string& getDefaultTpmScheme();

    /**
     * @brief Create a PIB according to @p pibLocator
     */
    static unique_ptr<Pib> createPib(const std::string& pibLocator);

    /**
     * @brief Create a TPM according to @p tpmLocator
     */
    static unique_ptr<Tpm> createTpm(const std::string& tpmLocator);

    NDN_CXX_PUBLIC_WITH_TESTS_ELSE_PRIVATE : static const std::string& getDefaultPibLocator();

    static const std::string& getDefaultTpmLocator();
  ...
  private: // signing
     ...
     */
    std::tuple<Name, SignatureInfo> prepareSignatureInfo(const SigningInfo& params);
     ...
     */
    Block sign(const uint8_t* buf, size_t size, const Name& keyName, DigestAlgorithm digestAlgorithm) const;
