...
class ValidationState : public TagHost, noncopyable {
  ...
  private: // Interface intended to be used only by Validator class
     ...
     */
    virtual void verifyOriginalPacket(const Certificate& trustedCert) = 0;

    /**
     * @brief Call success callback of the original packet without signature validation
     */
    virtual void bypassValidation() = 0;
     ...
     *       @p trustedCert.
     */
    const Certificate* verifyCertificateChain(const Certificate& trustedCert);
...
class DataValidationState final : public ValidationState {
  public:
     ...
     */
    DataValidationState(const Data& data, const DataValidationSuccessCallback& successCb,
  ...
  private:
    void verifyOriginalPacket(const Certificate& trustedCert) final;

    void bypassValidation() final;
...
class InterestValidationState final : public ValidationState {
  public:
     ...
     * after validateCertificateChain() and validateOriginalPacket() are called)
     */
    InterestValidationState(const Interest& interest, const InterestValidationSuccessCallback& successCb,
