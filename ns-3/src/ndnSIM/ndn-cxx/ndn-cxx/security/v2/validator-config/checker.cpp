...
NameRelationChecker::checkNames(const Name& pktName, const Name& klName, const shared_ptr<ValidationState>& state)
{
    ...
    if (!result) {
        ...
        os << "KeyLocator check failed: name relation " << m_name << " " << m_relation << " for packet " << pktName
           << " is invalid"
...
HyperRelationChecker::checkNames(const Name& pktName, const Name& klName, const shared_ptr<ValidationState>& state)
{
    if (!m_hyperPRegex.match(pktName) || !m_hyperKRegex.match(klName)) {
        std::ostringstream os;
        os << "Packet " << pktName << " ("
           << "KeyLocator=" << klName << ") does not match "
    ...
    if (!result) {
        std::ostringstream os;
        os << "KeyLocator check failed: hyper relation " << m_hyperRelation << " pkt=" << m_hyperPRegex
           << ", key=" << m_hyperKRegex << " of packet " << pktName << " (KeyLocator=" << klName << ") is invalid";
        state->fail({ValidationError::POLICY_ERROR, os.str()});
    }
    return result;
}
...
unique_ptr<Checker>
Checker::createCustomizedChecker(const ConfigSection& configSection, const std::string& configFilename)
{
    auto propertyIt = configSection.begin();
    propertyIt++;
...
unique_ptr<Checker>
Checker::createHierarchicalChecker(const ConfigSection& configSection, const std::string& configFilename)
{
    auto propertyIt = configSection.begin();
    propertyIt++;
    ...
    return make_unique<HyperRelationChecker>("^(<>*)$", "\\1", "^(<>*)<KEY><>$", "\\1", NameRelation::IS_PREFIX_OF);
}
