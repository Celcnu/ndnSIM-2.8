                                  ...
                                  const ValidationContinuation& continueValidation)
{
    ...
    if (bundleNameTag == nullptr) {
        ...
        fetchNextBundleSegment(fullBundleName, fullBundleName.get(-1).getSuccessor(), certRequest, state,
                               continueValidation);
    }
}
                                                  ...
                                                  const ValidationContinuation& continueValidation)
{
    ...
    m_face.expressInterest(
      ...
      [=](const Interest& interest) { timeoutCallback(certRequest, state, continueValidation, bundleNamePrefix); });
}
                                                 ...
                                                 const ValidationContinuation& continueValidation)
{
    ...
    m_face.expressInterest(
      ...
      [=](const Interest& interest) { timeoutCallback(certRequest, state, continueValidation, fullBundleName); });
}

void
CertificateBundleFetcher::dataCallback(const Data& bundleData, bool isSegmentZeroExpected,
                                       const shared_ptr<CertificateRequest>& certRequest,
                                       const shared_ptr<ValidationState>& state,
                                       const ValidationContinuation& continueValidation)
{
    ...
    if (isSegmentZeroExpected && currentSegment.toSegment() != 0) {
        // fetch segment zero
        fetchNextBundleSegment(bundleData.getName(), name::Component::fromSegment(0), certRequest, state,
                               continueValidation);
    }
    else {
...
void
CertificateBundleFetcher::nackCallback(const lp::Nack& nack, const shared_ptr<CertificateRequest>& certRequest,
                                       const shared_ptr<ValidationState>& state,
                                       const ValidationContinuation& continueValidation, const Name& bundleName)
{
    NDN_LOG_DEBUG_DEPTH("NACK (" << nack.getReason() << ") while fetching certificate bundle" << bundleName);
