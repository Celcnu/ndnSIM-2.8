...
ValidationPolicyConfig::load(std::istream& input, const std::string& filename)
{
    ...
    try {
        ...
        NDN_THROW(Error("Failed to parse configuration file " + filename + " line " + to_string(e.line()) + ": "
                        + e.message()));
    }
    load(tree, filename);
}
...
ValidationPolicyConfig::load(const ConfigSection& configSection, const std::string& filename)
{
    ...
    for (const auto& subSection : configSection) {
        ...
        if (boost::iequals(sectionName, "rule")) {
            ...
            NDN_THROW(
              Error("Error processing configuration file " + filename + ": unrecognized section " + sectionName));
        }
    }
}
...
ValidationPolicyConfig::processConfigTrustAnchor(const ConfigSection& configSection, const std::string& filename)
{
    ...
    if (boost::iequals(type, "file")) {
        ...
        m_validator->loadAnchor(filename, absolute(file, path(filename).parent_path()).string(), refresh, false);
    }
...
time::nanoseconds
ValidationPolicyConfig::getRefreshPeriod(ConfigSection::const_iterator& it, const ConfigSection::const_iterator& end)
{
                                    ...
                                    const ValidationContinuation& continueValidation)
{
    ...
    return state->fail(
      {ValidationError::POLICY_ERROR, "No rule matched for interest `" + interest.getName().toUri() + "`"});
}
