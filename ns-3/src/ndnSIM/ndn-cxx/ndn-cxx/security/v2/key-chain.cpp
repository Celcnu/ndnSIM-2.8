...
KeyChain::importSafeBag(const SafeBag& safeBag, const char* pw, size_t pwLen)
{
    ...
    try {
        m_tpm->importPrivateKey(keyName, safeBag.getEncryptedKeyBag().data(), safeBag.getEncryptedKeyBag().size(), pw,
                                pwLen);
    }
    catch (const Tpm::Error&) {
    ...
    bool isVerified = false;
    {
        ...
        bufferSource(content, sizeof(content))
          >> verifierFilter(DigestAlgorithm::SHA256, publicKey, sigBits->data(), sigBits->size())
          >> boolSink(isVerified);
    }
    if (!isVerified) {
        m_tpm->deleteKey(keyName);
...
KeyChain::sign(Interest& interest, const SigningInfo& params)
{
    ...
    Block sigValue =
      sign(signedName.wireEncode().value(), signedName.wireEncode().value_size(), keyName, params.getDigestAlgorithm());
...
KeyChain::selfSign(Key& key)
{
    ...
    signatureInfo.setValidityPeriod(
      ValidityPeriod(time::system_clock::TimePoint(), time::system_clock::now() + 20 * 365_days));
    ...
    switch (params.getSignerType()) {
        ...
        case SigningInfo::SIGNER_TYPE_ID: {
            ...
            if (!identity) {
                try {
                    ...
                    NDN_THROW_NESTED(InvalidSigningInfoError("Signing identity `" + params.getSignerName().toUri()
                                                             + "` does not exist"));
                }
            }
            break;
        ...
        case SigningInfo::SIGNER_TYPE_KEY: {
            ...
            if (!key) {
                ...
                try {
                    ...
                    NDN_THROW_NESTED(
                      InvalidSigningInfoError("Signing key `" + params.getSignerName().toUri() + "` does not exist"));
                }
            }
            break;
        }
        case SigningInfo::SIGNER_TYPE_CERT: {
            ...
            try {
                ...
                NDN_THROW_NESTED(InvalidSigningInfoError("Signing certificate `" + params.getSignerName().toUri()
                                                         + "` does not exist"));
            }
            break;
        }
...
Block
KeyChain::sign(const uint8_t* buf, size_t size, const Name& keyName, DigestAlgorithm digestAlgorithm) const
{
