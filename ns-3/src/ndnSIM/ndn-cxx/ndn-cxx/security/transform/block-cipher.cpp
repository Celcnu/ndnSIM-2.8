...
BlockCipher::BlockCipher(BlockCipherAlgorithm algo, CipherOperator op, const uint8_t* key, size_t keyLen,
  ...
  : m_impl(make_unique<Impl>())
{
    switch (algo) {
        ...
        default:
            NDN_THROW(
              Error(getIndex(), "Unsupported block cipher algorithm " + boost::lexical_cast<std::string>(algo)));
    }
}
...
void
BlockCipher::initializeAesCbc(const uint8_t* key, size_t keyLen, const uint8_t* iv, size_t ivLen, CipherOperator op)
{
