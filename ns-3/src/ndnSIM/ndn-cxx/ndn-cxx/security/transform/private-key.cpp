...
PrivateKey::getKeyDigest(DigestAlgorithm algo) const
{
    if (getKeyType() != KeyType::HMAC)
        NDN_THROW(Error("Digest is not supported for key type " + boost::lexical_cast<std::string>(getKeyType())));
...
PrivateKey::toPkcs8(const char* pw, size_t pwLen) const
{
    ...
    detail::Bio membio(BIO_s_mem());
    if (!i2d_PKCS8PrivateKey_bio(membio, m_impl->key, EVP_aes_256_cbc(), nullptr, 0, nullptr, const_cast<char*>(pw)))
...
PrivateKey::toPkcs8(PasswordCallback pwCallback) const
{
    ...
    detail::Bio membio(BIO_s_mem());
    if (!i2d_PKCS8PrivateKey_bio(membio, m_impl->key, EVP_aes_256_cbc(), nullptr, 0, &passwordCallbackWrapper,
                                 &pwCallback))
        NDN_THROW(Error("Cannot convert key to PKCS #8 format"));
