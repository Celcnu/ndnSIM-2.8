...
bool
verifySignature(const uint8_t* blob, size_t blobLen, const uint8_t* sig, size_t sigLen, const transform::PublicKey& key)
{
    bool result = false;
    try {
        using namespace transform;
        bufferSource(blob, blobLen) >> verifierFilter(DigestAlgorithm::SHA256, key, sig, sigLen) >> boolSink(result);
    }
...
bool
verifySignature(const uint8_t* data, size_t dataLen, const uint8_t* sig, size_t sigLen, const uint8_t* key,
                size_t keyLen)
{
...
{
    try {
        return {true, data.wireEncode().value(), data.wireEncode().value_size() - data.getSignature().getValue().size(),
...
parse(const Interest& interest)
{
    ...
    try {
        ...
        return {true, nameBlock.value(), nameBlock.value_size() - interestName[signed_interest::POS_SIG_VALUE].size(),
...
{
    return params.isParsable && verifySignature(params.buf, params.bufLen, params.sig, params.sigLen, key);
}

static bool
verifySignature(ParseResult params, const tpm::Tpm& tpm, const Name& keyName, DigestAlgorithm digestAlgorithm)
{
    return params.isParsable
           && bool(tpm.verify(params.buf, params.bufLen, params.sig, params.sigLen, keyName, digestAlgorithm));
}

static bool
verifySignature(ParseResult params, const uint8_t* key, size_t keyLen)
{
    return params.isParsable && verifySignature(params.buf, params.bufLen, params.sig, params.sigLen, key, keyLen);
}
...
bool
verifySignature(const Data& data, const tpm::Tpm& tpm, const Name& keyName, DigestAlgorithm digestAlgorithm)
{
...
verifyDigest(const Data& data, DigestAlgorithm algorithm)
{
    ...
    return parseResult.isParsable
           && verifyDigest(parseResult.buf, parseResult.bufLen, parseResult.sig, parseResult.sigLen, algorithm);
}
