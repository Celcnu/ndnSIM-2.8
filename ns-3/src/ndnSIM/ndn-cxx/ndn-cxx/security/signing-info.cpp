...
SigningInfo::SigningInfo(SignerType signerType, const Name& signerName, const SignatureInfo& signatureInfo)
  : m_type(signerType)
  , m_name(signerName)
...
SigningInfo::setSigningHmacKey(const std::string& hmacKey)
{
    ...
    OBufferStream os;
    transform::bufferSource(hmacKey) >> transform::base64Decode(false) >> transform::streamSink(os);
