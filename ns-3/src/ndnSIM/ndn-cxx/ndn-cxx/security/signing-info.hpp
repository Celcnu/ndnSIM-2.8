...
class SigningInfo {
  ...
  public:
     ...
     */
    explicit SigningInfo(SignerType signerType = SIGNER_TYPE_NULL, const Name& signerName = getEmptyName(),
                         const SignatureInfo& signatureInfo = getEmptySignatureInfo());

    /**
     * @brief Construct from a PIB identity.
     */
    explicit SigningInfo(const Identity& identity);

    /**
     * @brief Construct from a PIB key.
     */
    explicit SigningInfo(const Key& key);
     ...
     */
    explicit SigningInfo(const std::string& signingStr);
  ...
  private: // non-member operators
    ...
    operator!=(const SigningInfo& lhs, const SigningInfo& rhs)
    {
        return lhs.m_type != rhs.m_type || lhs.m_name != rhs.m_name || lhs.m_digestAlgorithm != rhs.m_digestAlgorithm
