...
class DelegationList {
  public:
     ...
     *  \sa wireDecode
     */
    explicit DelegationList(const Block& block, bool wantSort = true);
  ...
  private:
    static bool isValidTlvType(uint32_t type);

    void insertImpl(uint64_t preference, const Name& name);

    size_t eraseImpl(optional<uint64_t> preference, const Name& name);
...
#ifndef DOXYGEN
extern template size_t DelegationList::wireEncode<encoding::EncoderTag>(EncodingBuffer&, uint32_t) const;

extern template size_t DelegationList::wireEncode<encoding::EstimatorTag>(EncodingEstimator&, uint32_t) const;
#endif
