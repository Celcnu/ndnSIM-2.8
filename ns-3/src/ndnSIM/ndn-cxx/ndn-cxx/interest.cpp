...
Interest::isParametersDigestValid() const
{
    ...
    return std::equal(digestComponent.value_begin(), digestComponent.value_end(), digest->begin(), digest->end());
}
...
Interest::computeParametersDigest() const
{
    ...
    std::for_each(m_parameters.begin(), m_parameters.end(), [&](const Block& b) { in.write(b.wire(), b.size()); });
    in.end();
