...
Address::toString(char sep) const
{
    ...
    snprintf(s, sizeof(s), "%02x%c%02x%c%02x%c%02x%c%02x%c%02x", at(0), sep, at(1), sep, at(2), sep, at(3), sep, at(4),
             sep, at(5));
...
Address::fromString(const std::string& str)
{
    ...
    // apparently gcc-4.6 does not support the 'hh' type modifier
    int ret =
      std::sscanf(str.c_str(), "%2hx%1[:-]%2hx%1[:-]%2hx%1[:-]%2hx%1[:-]%2hx%1[:-]%2hx%n", &temp[0], &sep[0][0],
                  &temp[1], &sep[1][0], &temp[2], &sep[2][0], &temp[3], &sep[3][0], &temp[4], &sep[4][0], &temp[5], &n);
