...
class NetlinkMessage {
  public:
    explicit NetlinkMessage(const uint8_t* buf, size_t buflen) noexcept
...
class NetlinkMessageAttributes {
  ...
  public:
    explicit NetlinkMessageAttributes(const T* begin, size_t length) noexcept
    {
    ...
    getAttributeByType(uint16_t attrType) const
    {
        ...
        return convertAttrValue(getAttributeValue(it->second), getAttributeValueLength(it->second),
                                AttrValueTypeTag<U>{});
    }
  ...
  private:
    ...
    {
        return attr != nullptr && nBytesRemaining >= sizeof(T) && getAttributeLength(attr) >= sizeof(T)
