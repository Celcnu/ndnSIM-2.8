  ...
  , m_genlSocket(io)
{
    ...
    for (auto group :
         {RTNLGRP_LINK, RTNLGRP_IPV4_IFADDR, RTNLGRP_IPV4_ROUTE, RTNLGRP_IPV6_IFADDR, RTNLGRP_IPV6_ROUTE}) {
...
NetworkMonitorImplNetlink::parseAddressMessage(const NetlinkMessage& nlmsg)
{
    ...
    NetworkAddress address(ifaFamilyToAddressFamily(ifa->ifa_family), ipAddr, broadcastAddr, ifa->ifa_prefixlen,
                           ifaScopeToAddressScope(ifa->ifa_scope), flags);
...
NetworkMonitorImplNetlink::parseErrorMessage(const NetlinkMessage& nlmsg)
{
    ...
    if (err->error != 0)
        NDN_LOG_ERROR("NLMSG_ERROR for seq=" << err->msg.nlmsg_seq << " err=" << err->error << " "
