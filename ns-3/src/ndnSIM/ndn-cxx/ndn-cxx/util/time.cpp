...
void
setCustomClocks(shared_ptr<CustomSteadyClock> steadyClock, shared_ptr<CustomSystemClock> systemClock)
{
...
std::string
toString(const system_clock::TimePoint& timePoint, const std::string& format /* = "%Y-%m-%d %H:%M:%S"*/,
...
system_clock::TimePoint
fromString(const std::string& timePointStr, const std::string& format /* = "%Y-%m-%d %H:%M:%S"*/,
           const std::locale& locale /* = std::locale("C")*/)
{
    namespace bpt = boost::posix_time;
