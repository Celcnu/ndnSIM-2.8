...
RttEstimator::addMeasurement(time::nanoseconds rtt, size_t nExpectedSamples)
{
    ...
    if (!hasSamples()) { // first measurement
        ...
        m_rttVar = time::duration_cast<time::nanoseconds>((1 - beta) * m_rttVar + beta * time::abs(m_sRtt - rtt));
        m_sRtt = time::duration_cast<time::nanoseconds>((1 - alpha) * m_sRtt + alpha * rtt);
    }
    m_rto = clamp(m_sRtt + m_options->k * m_rttVar, m_options->minRto, m_options->maxRto);
}

void
RttEstimator::backoffRto()
{
    m_rto = clamp(m_rto * m_options->rtoBackoffMultiplier, m_options->minRto, m_options->maxRto);
}
