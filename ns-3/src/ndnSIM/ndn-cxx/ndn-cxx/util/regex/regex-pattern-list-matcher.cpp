...
RegexPatternListMatcher::extractPattern(size_t index, size_t* next)
{
    ...
    switch (m_expr[index]) {
        case '(':
            ...
            if (indicator == end) {
                auto matcher = make_shared<RegexBackrefMatcher>(m_expr.substr(start, end - start), m_backrefManager);
        ...
        case '<':
            ...
            m_matchers.push_back(
              make_shared<RegexRepeatMatcher>(m_expr.substr(start, end - start), m_backrefManager, indicator - start));
            break;
