...
union storage_t {
    ...
    void
    construct_value(value_type const& v)
    {
    ...
    optional_nodiscard value_type const*
    value_ptr() const
    {
...
#undef optional_ALIGN_AS
    ...
    optional_nodiscard void*
    ptr() optional_noexcept
    {
        return &data;
    }

    optional_nodiscard void const*
    ptr() const optional_noexcept
    {
        return &data;
    }

    template <typename U>
    optional_nodiscard U*
    as()
    {
...
class optional {
  ...
  public:
    ...
    optional_constexpr
    optional() optional_noexcept : has_value_(false),
                                   contained()
    {
    }
    ...
    optional_constexpr14
    optional(optional&& other optional_REQUIRES_A(true || std::is_move_constructible<T>::value)
    ...
    // NOLINTNEXTLINE( google-explicit-constructor, hicpp-explicit-conversions )
    optional_constexpr
    optional(U&& value optional_REQUIRES_A(
      std::is_constructible<T, U&&>::value
      && !std::is_same<typename std20::remove_cvref<U>::type, nonstd_lite_in_place_t(U)>::value
      && !std::is_same<typename std20::remove_cvref<U>::type, optional<T>>::value
    ...
    optional_REQUIRES_R(
      ...
      std::is_constructible<T, U const&>::value&& std::is_assignable<T&, U const&>::value
        ...
        && !std::is_assignable<T&, optional<U> const&>::value && !std::is_assignable<T&, optional<U> const&&>::value)
#else
    optional&
#endif // optional_CPP11_OR_GREATER
    ...
    value_or(U&& v) const optional_ref_qual
    {
    ...
    template <typename U>
    optional_constexpr14 value_type
    value_or(U&& v) optional_refref_qual
    {
