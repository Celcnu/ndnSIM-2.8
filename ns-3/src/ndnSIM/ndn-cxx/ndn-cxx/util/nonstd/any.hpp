...
#define any_REQUIRES_T(...) , typename = typename std::enable_if<(__VA_ARGS__), nonstd::any_lite::detail::enabler>::type
...
class any {
  public:
    any_constexpr
    any() any_noexcept : content(any_nullptr)
    {
    }
    ...
    any&
    operator=(any&& other) any_noexcept
    {
    ...
    void
    swap(any& other) any_noexcept
    {
        std::swap(content, other.content);
    }

    bool
    has_value() const any_noexcept
    {
        return content != any_nullptr;
    }

    const std::type_info&
    type() const any_noexcept
    {
