...
class DummyClientFace : public ndn::Face {
  public:
     ...
     */
    explicit DummyClientFace(const Options& options = Options());

    /** \brief Create a dummy face with internal IO service and the specified KeyChain
     */
    explicit DummyClientFace(KeyChain& keyChain, const Options& options = Options());

    /** \brief Create a dummy face with the provided IO service
     */
    explicit DummyClientFace(boost::asio::io_service& ioService, const Options& options = Options());
  ...
  private:
    ...
    void construct(const Options& options);

    void enableBroadcastLink();

    void enablePacketLogging();

    void enableRegistrationReply();

    void doProcessEvents(time::milliseconds timeout, bool keepThread) override;
  ...
  public:
    ...
    NDN_CXX_PUBLIC_WITH_TESTS_ELSE_PRIVATE : struct BroadcastLink;
