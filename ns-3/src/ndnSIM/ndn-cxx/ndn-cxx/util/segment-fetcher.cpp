...
SegmentFetcher::SegmentFetcher(Face& face, security::v2::Validator& validator, const SegmentFetcher::Options& options)
  : m_options(options)
  , m_face(face)
...
SegmentFetcher::start(Face& face, const Interest& baseInterest, security::v2::Validator& validator,
...
SegmentFetcher::sendInterest(uint64_t segNum, const Interest& interest, bool isRetransmission)
{
    ...
    ++m_nSegmentsInFlight;
    auto pendingInterest = m_face.expressInterest(
      interest,
    ...
    auto timeoutEvent =
      m_scheduler.schedule(timeout, [this, interest, weakSelf] { afterTimeoutCb(interest, weakSelf); });
    ...
    PendingSegment pendingSegment{SegmentState::FirstInterest, time::steady_clock::now(), pendingInterest,
                                  timeoutEvent};
                                       ...
                                       const weak_ptr<SegmentFetcher>& weakSelf)
{
    ...
    m_validator.validate(data,
                         bind(&SegmentFetcher::afterValidationSuccess, this, _1, origInterest, pendingSegmentIt,
                              weakSelf),
                                       ...
                                       const weak_ptr<SegmentFetcher>& weakSelf)
{
    ...
    auto receivedSegmentIt = m_receivedSegments.emplace(std::piecewise_construct, std::forward_as_tuple(currentSegment),
                                                        std::forward_as_tuple(data.getContent().value_size()));
    std::copy(data.getContent().value_begin(), data.getContent().value_end(), receivedSegmentIt.first->second.begin());
    ...
    if (data.getFinalBlock()) {
        if (!data.getFinalBlock()->isSegment()) {
            return signalError(FINALBLOCKID_NOT_SEGMENT, "Received FinalBlockId did not contain a segment component");
        }
...
void
SegmentFetcher::afterTimeoutCb(const Interest& origInterest, const weak_ptr<SegmentFetcher>& weakSelf)
{
    if (shouldStop(weakSelf))
        return;
...
void
SegmentFetcher::updateRetransmittedSegment(uint64_t segmentNum, const PendingInterestHandle& pendingInterest,
...
SegmentFetcher::getEstimatedRto()
{
    ...
    return std::min(m_options.maxTimeout, time::duration_cast<time::milliseconds>(m_rttEstimator.getEstimatedRto()));
}
