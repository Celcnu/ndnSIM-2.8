...
Component::fromEscapedString(const std::string& input)
{
    ...
    if (type >= tlv::NameComponentMin && type <= tlv::NameComponentMax && to_string(type) == typePrefix) {
        size_t valuePos = equalPos + 1;
        return parseUriEscapedValue(static_cast<uint32_t>(type), input.data() + valuePos, input.size() - valuePos);
    }
...
{
    return value_size() == 1 || value_size() == 2 || value_size() == 4 || value_size() == 8;
}

bool
Component::isNumberWithMarker(uint8_t marker) const
{
    return (value_size() == 2 || value_size() == 3 || value_size() == 5 || value_size() == 9) && value()[0] == marker;
}
...
Component::isByteOffset() const
{
    return (canDecodeMarkerConvention() && type() == tlv::GenericNameComponent
            && isNumberWithMarker(SEGMENT_OFFSET_MARKER))
...
{
    return (canDecodeMarkerConvention() && type() == tlv::GenericNameComponent
            && isNumberWithMarker(SEQUENCE_NUMBER_MARKER))
...
{
    return g_conventionEncoding == Convention::MARKER ? fromNumberWithMarker(VERSION_MARKER, version)
...
{
    return g_conventionEncoding == Convention::MARKER ? fromNumberWithMarker(SEGMENT_MARKER, segmentNo)
...
{
    return g_conventionEncoding == Convention::MARKER ? fromNumberWithMarker(SEGMENT_OFFSET_MARKER, offset)
...
Component::fromTimestamp(const time::system_clock::TimePoint& timePoint)
{
    ...
    return g_conventionEncoding == Convention::MARKER ? fromNumberWithMarker(TIMESTAMP_MARKER, value)
...
{
    return g_conventionEncoding == Convention::MARKER ? fromNumberWithMarker(SEQUENCE_NUMBER_MARKER, seqNo)
...
{
    return type() == other.type() && value_size() == other.value_size()
