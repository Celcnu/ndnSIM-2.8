...
class PendingInterest : public RecordBase<PendingInterest> {
  public:
    ...
    PendingInterest(shared_ptr<const Interest> interest, const DataCallback& dataCallback,
                    const NackCallback& nackCallback, const TimeoutCallback& timeoutCallback, Scheduler& scheduler)
  ...
  private:
    ...
    {
        m_timeoutEvent = scheduler.schedule(m_interest->getInterestLifetime(), [=] { this->invokeTimeoutCallback(); });
    }
