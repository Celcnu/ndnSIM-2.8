...
class NdnGlobalRouterGraph {
  public:
    ...
    NdnGlobalRouterGraph()
    {
        ...
        for (ns3::ChannelList::Iterator channel = ns3::ChannelList::Begin(); channel != ns3::ChannelList::End();
             channel++) {
...
class ndn_global_router_graph_category : public virtual vertex_list_graph_tag, public virtual incidence_graph_tag {
};
...
const property_traits<EdgeWeights>::value_type WeightInf(nullptr, std::numeric_limits<uint16_t>::max(), 0.0);
...
struct WeightCombine : public std::binary_function<uint32_t, property_traits<EdgeWeights>::reference, uint32_t> {
    uint32_t
...
get(const boost::EdgeWeights&, ns3::ndn::GlobalRouter::Incidency& edge)
{
    if (std::get<1>(edge) == 0)
        ...
        return property_traits<EdgeWeights>::reference(std::get<1>(edge),
                                                       static_cast<uint16_t>(std::get<1>(edge)->getMetric()), 0.0);
    }
}

struct PredecessorsMap : public std::map<ns3::Ptr<ns3::ndn::GlobalRouter>, ns3::Ptr<ns3::ndn::GlobalRouter>> {
};
...
struct DistancesMap
  : public std::map<ns3::Ptr<ns3::ndn::GlobalRouter>, std::tuple<std::shared_ptr<nfd::Face>, uint32_t, double>> {
};
...
get(DistancesMap& map, ns3::Ptr<ns3::ndn::GlobalRouter> key)
{
    ...
    if (i == map.end())
        return std::tuple<std::shared_ptr<nfd::Face>, uint32_t, double>(nullptr, std::numeric_limits<uint32_t>::max(),
                                                                        0.0);
    else
        return i->second;
}
