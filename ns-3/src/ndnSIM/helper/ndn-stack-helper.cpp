...
void
StackHelper::SetStackAttributes(const std::string& attr1, const std::string& value1, const std::string& attr2,
...
void
StackHelper::AddFaceCreateCallback(TypeId netDeviceType, StackHelper::FaceCreateCallback callback)
{
    m_netDeviceCallbacks.push_back(std::make_pair(netDeviceType, callback));
}

void
StackHelper::UpdateFaceCreateCallback(TypeId netDeviceType, FaceCreateCallback callback)
{
...
void
StackHelper::RemoveFaceCreateCallback(TypeId netDeviceType, FaceCreateCallback callback)
{
    m_netDeviceCallbacks.remove_if(
      [&](const std::pair<TypeId, FaceCreateCallback>& i) { return (i.first == netDeviceType); });
}
...
StackHelper::DefaultNetDeviceCallback(Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> netDevice) const
{
    ...
    auto transport =
      make_unique<NetDeviceTransport>(node, netDevice, constructFaceUri(netDevice), "netdev://[ff:ff:ff:ff:ff:ff]");
...
StackHelper::PointToPointNetDeviceCallback(Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const
{
    ...
    auto transport =
      make_unique<NetDeviceTransport>(node, netDevice, constructFaceUri(netDevice), constructFaceUri(remoteNetDevice));
...
StackHelper::createAndRegisterFace(Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const
{
    ...
    for (const auto& item : m_netDeviceCallbacks) {
        if (device->GetInstanceTypeId() == item.first || device->GetInstanceTypeId().IsChildOf(item.first)) {
...
StackHelper::SetLinkDelayAsFaceMetric()
{
    for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i) {
        ...
        for (auto& face : ndn->getFaceTable()) {
            ...
            std::cout << "Node " << i << ": Face " << face.getId() << " with metric " << face.getMetric() << "\n";
        }
    }
}
