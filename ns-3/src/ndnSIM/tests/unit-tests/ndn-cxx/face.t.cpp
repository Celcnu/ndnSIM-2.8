...
BOOST_AUTO_TEST_CASE(SetInterestFilter)
{
    FactoryCallbackApp::Install(getNode("B"), [this]() -> shared_ptr<void> {
        return make_shared<BasicProducer>(
          "/test",
          [this](const Name& interest) {
    ...
    addApps({{"A", "ns3::ndn::ConsumerBatches", {{"Prefix", "/test/prefix"}, {"Batches", "0s 1"}}, "1s", "5.1s"}});
...
class SingleInterest : public BaseTesterApp {
  public:
    SingleInterest(const Name& name, const std::function<void(const Data&)>& onData, const VoidCallback& onNack,
                   const VoidCallback& onTimeout)
    {
        m_face.expressInterest(Interest(name).setCanBePrefix(true),
                               std::bind([onData](const Data& data) { onData(data); }, _2), std::bind(onNack),
                               std::bind(onTimeout));
    }
};
...
BOOST_AUTO_TEST_CASE(ExpressInterestLocalhost)
{
    ...
    FactoryCallbackApp::Install(getNode("A"), [this]() -> shared_ptr<void> {
        return make_shared<SingleInterest>(
          "/localhost",
          ...
          [] { BOOST_ERROR("Unexpected NACK"); }, [] { BOOST_ERROR("Unexpected timeout"); });
    }).Start(Seconds(1.01));
...
BOOST_AUTO_TEST_CASE(ExpressInterestTimeout)
{
    FactoryCallbackApp::Install(getNode("A"), [this]() -> shared_ptr<void> {
        return make_shared<SingleInterest>(
          Name("/test/prefix"), [](const Data&) { BOOST_ERROR("Unexpected data"); },
...
class MultipleInterest : public BaseTesterApp {
  ...
  private:
    ...
    expressNextInterest(const Name& name, uint32_t seqNo, const NameCallback& onData, const VoidCallback& onTimeout,
                        const VoidCallback& onNack)
    {
        m_face.expressInterest(Interest(Name(name).appendSegment(seqNo)).setCanBePrefix(true),
                               std::bind(
                                 [=](const Data& data) {
                                     onData(data.getName());
                                 ...
                                 _2),
                               std::bind(onNack), std::bind(onTimeout));
    }
...
BOOST_AUTO_TEST_CASE(ExpressMultipleInterests)
{
    ...
    FactoryCallbackApp::Install(getNode("A"), [&recvCount]() -> shared_ptr<void> {
        return make_shared<MultipleInterest>(
          "/test/prefix",
          [&recvCount](const Name& data) {
              BOOST_CHECK_EQUAL(data, Name("/test/prefix").appendSegment(recvCount));
              ++recvCount;
          },
          [] { BOOST_ERROR("Unexpected timeout"); }, [] { BOOST_ERROR("Unexpected NACK"); });
    }).Start(Seconds(1.01));
  ...
  public:
    SingleInterestWithFaceShutdown()
    {
        m_face.expressInterest(Interest(Name("/interest/to/timeout")).setCanBePrefix(true),
                               std::bind([] { BOOST_ERROR("Unexpected response"); }),
                               std::bind([this] { m_face.shutdown(); }), std::bind([this] { m_face.shutdown(); }));
    }
};
