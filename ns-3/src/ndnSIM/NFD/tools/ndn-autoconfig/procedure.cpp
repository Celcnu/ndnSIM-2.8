          ...
          m_controller.start<nfd::FaceCreateCommand>(
            ...
            [this, canonicalUri](const ControlResponse& resp) {
                if (resp.getCode() == 409) {
                    ...
                    std::cerr << "Failed to connect to HUB " << canonicalUri << ": " << resp.getText() << " ("
                              << resp.getCode() << ")" << std::endl;
...
Procedure::registerPrefixes(uint64_t hubFaceId, size_t index)
{
    ...
    m_controller.start<nfd::RibRegisterCommand>(
      ...
      [=](const ControlResponse& resp) {
          std::cerr << "Failed to register " << HUB_PREFIXES[index] << ": " << resp.getText() << " (" << resp.getCode()
                    << ")" << std::endl;
          this->onComplete(false);
      });
}
