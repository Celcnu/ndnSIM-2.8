...
class AutoregServer : boost::noncopyable {
  public:
    ...
    void
    onRegisterCommandFailure(uint64_t faceId, const Name& prefix, const nfd::ControlResponse& response)
    {
        std::cerr << "FAILED: register " << prefix << " on face " << faceId << " (code: " << response.getCode()
                  << ", reason: " << response.getText() << ")" << std::endl;
    }
    ...
    hasAllowedSchema(const FaceUri& uri)
    {
        ...
        return scheme == "udp4" || scheme == "tcp4" || scheme == "udp6" || scheme == "tcp6";
    }
    ...
    {
        return std::any_of(m_blackList.begin(), m_blackList.end(), bind(&Network::doesContain, _1, address));
    }
    ...
    {
        return std::any_of(m_whiteList.begin(), m_whiteList.end(), bind(&Network::doesContain, _1, address));
    }
    ...
    registerPrefixesIfNeeded(uint64_t faceId, const FaceUri& uri, nfd::FacePersistency facePersistency)
    {
        if (hasAllowedSchema(uri)) {
            ...
            if (!address.is_multicast()) {
                ...
                if (facePersistency == nfd::FACE_PERSISTENCY_ON_DEMAND && !isBlacklisted(address)
    ...
    {
        if (notification.getKind() == nfd::FACE_EVENT_CREATED && notification.getFaceScope() != nfd::FACE_SCOPE_LOCAL) {
    ...
    static void
    usage(std::ostream& os, const boost::program_options::options_description& desc, const char* programName)
    {
    ...
    main(int argc, char* argv[])
    {
        ...
        optionsDesc
          .add_options()("help,h", "print this message and exit")(
            "version,V",
            "show version information and exit")("prefix,i",
                                                 po::value<std::vector<Name>>(&m_autoregPrefixes)->composing(),
                                                 "prefix that should be automatically registered when a new non-local "
                                                 "face is created")("all-faces-prefix,a",
                                                                    po::value<std::vector<Name>>(&m_allFacesPrefixes)
                                                                      ->composing(),
                                                                    "prefix that should be automatically registered "
                                                                    "for all TCP and UDP non-local faces "
                                                                    "(blacklists and whitelists do not apply to this "
                                                                    "prefix)")("cost,c",
                                                                               po::value<uint64_t>(&m_cost)
                                                                                 ->default_value(255),
                                                                               "FIB cost that should be assigned to "
                                                                               "autoreg nexthops")("whitelist,w",
                                                                                                   po::value<
                                                                                                     std::vector<
                                                                                                       Network>>(
                                                                                                     &m_whiteList)
        ...
        if (m_autoregPrefixes.empty() && m_allFacesPrefixes.empty()) {
            std::cerr << "ERROR: at least one --prefix or --all-faces-prefix must be specified" << std::endl
                      << std::endl;
            usage(std::cerr, optionsDesc, argv[0]);
            return 2;
        }
