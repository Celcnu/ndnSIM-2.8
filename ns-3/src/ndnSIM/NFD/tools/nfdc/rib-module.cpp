...
RibModule::registerCommands(CommandParser& parser)
{
    ...
    defRouteList.setTitle("print RIB routes")
    ...
    defRouteShow.setTitle("show routes toward a prefix")
    ...
    defRouteAdd.setTitle("add a route")
      .addArg("prefix", ArgValueType::NAME, Required::YES, Positional::YES)
    ...
    defRouteRemove.setTitle("remove a route")
      .addArg("prefix", ArgValueType::NAME, Required::YES, Positional::YES)
      .addArg("nexthop", ArgValueType::FACE_ID_OR_URI, Required::YES, Positional::YES)
      .addArg("origin", ArgValueType::ROUTE_ORIGIN, Required::NO, Positional::NO);
...
RibModule::list(ExecuteContext& ctx)
{
    ...
    listRoutesImpl(ctx, [&](const RibEntry& entry, const Route& route) {
        return (nexthops.empty() || nexthops.count(route.getFaceId()) > 0) && (!origin || route.getOrigin() == *origin);
    });
}
...
RibModule::show(ExecuteContext& ctx)
{
    ...
    listRoutesImpl(ctx, [&](const RibEntry& entry, const Route& route) { return entry.getName() == prefix; });
}
...
RibModule::listRoutesImpl(ExecuteContext& ctx, const RoutePredicate& filter)
{
    ctx.controller.fetch<ndn::nfd::RibDataset>(
      ...
      ctx.makeDatasetFailureHandler("RIB dataset"), ctx.makeCommandOptions());
...
RibModule::add(ExecuteContext& ctx)
{
    ...
    ctx.controller.start<ndn::nfd::RibRegisterCommand>(
      ...
      [&](const ControlParameters& resp) {
          ...
          ctx.out << ia("prefix") << resp.getName() << ia("nexthop") << resp.getFaceId() << ia("origin")
                  << resp.getOrigin() << ia("cost") << resp.getCost() << ia("flags")
                  << static_cast<ndn::nfd::RouteFlags>(resp.getFlags());
      ...
      ctx.makeCommandFailureHandler("adding route"), ctx.makeCommandOptions());
...
RibModule::remove(ExecuteContext& ctx)
{
    ...
    for (uint64_t faceId : findFace.getFaceIds()) {
        ...
        unregisterParams.setName(prefix).setFaceId(faceId).setOrigin(origin);
        ...
        ctx.controller.start<ndn::nfd::RibUnregisterCommand>(
          ...
          ctx.makeCommandFailureHandler("removing route"), ctx.makeCommandOptions());
    }
...
void
RibModule::fetchStatus(Controller& controller, const std::function<void()>& onSuccess,
...
void
RibModule::formatRouteText(std::ostream& os, const RibEntry& entry, const Route& route, bool includePrefix)
{
    text::ItemAttributes ia;
