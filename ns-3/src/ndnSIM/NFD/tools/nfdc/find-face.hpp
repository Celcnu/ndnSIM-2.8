...
class FindFace : noncopyable {
  public:
    ...
    explicit FindFace(ExecuteContext& ctx);
     ...
     */
    Code execute(const ndn::any& faceIdOrUri, bool allowMulti = false);
  ...
  private:
     ...
     */
    optional<FaceUri> canonize(const std::string& fieldName, const FaceUri& input);
