...
FaceModule::registerCommands(CommandParser& parser)
{
    ...
    defFaceList.setTitle("print face list")
    ...
    defFaceShow.setTitle("show face information").addArg("id", ArgValueType::UNSIGNED, Required::YES, Positional::YES);
    parser.addCommand(defFaceShow, &FaceModule::show);

    CommandDefinition defFaceCreate("face", "create");
    defFaceCreate.setTitle("create a face")
    ...
    defFaceDestroy.setTitle("destroy a face")
...
FaceModule::create(ExecuteContext& ctx)
{
    ...
    auto printPositiveResult = [&](const std::string& actionSummary, const ControlParameters& resp) {
        ...
        ctx.out << actionSummary << ' ' << ia("id") << resp.getFaceId() << ia("local") << resp.getLocalUri()
                << ia("remote") << resp.getUri() << ia("persistency") << resp.getFacePersistency();
    ...
    auto updateFace = [&printPositiveResult](ControlParameters respParams, ControlParameters resp) {
        ...
        resp.setLocalUri(respParams.getLocalUri()).setUri(respParams.getUri());
    ...
    auto handle409 = [&](const ControlResponse& resp) {
        ...
        if (mtu && (!respParams.hasMtu() || respParams.getMtu() != *mtu)) {
            ...
            ctx.controller.start<ndn::nfd::FaceUpdateCommand>(params, bind(updateFace, respParams, _1),
                                                              ctx.makeCommandFailureHandler(
                                                                "upgrading face persistency"),
                 ...
                 || baseCongestionMarkingIntervalMs || defaultCongestionThreshold) {
            ControlParameters params;
    ...
    auto doCreateFace = [&] {
        ...
        ctx.controller.start<ndn::nfd::FaceCreateCommand>(
          params, bind(printPositiveResult, "face-created", _1),
      ...
      [&](const FaceUri& canonicalUri) {
          ...
          if (localUri) {
              localUri->canonize(
                ...
                bind(handleCanonizeError, *localUri, _1), ctx.face.getIoService(), ctx.getTimeout());
          }
          else {
              doCreateFace();
          }
      },
      bind(handleCanonizeError, remoteUri, _1), ctx.face.getIoService(), ctx.getTimeout());
...
FaceModule::destroy(ExecuteContext& ctx)
{
    ...
    ctx.controller.start<ndn::nfd::FaceDestroyCommand>(
      ...
      [&](const ControlParameters& resp) {
          ...
          text::ItemAttributes ia;
          ctx.out << ia("id") << face.getFaceId() << ia("local") << face.getLocalUri() << ia("remote")
                  << face.getRemoteUri() << ia("persistency") << face.getFacePersistency();
          printFaceParams(ctx.out, ia, resp);
      },
      ctx.makeCommandFailureHandler("destroying face"), ctx.makeCommandOptions());

    ctx.face.processEvents();
}

void
FaceModule::fetchStatus(Controller& controller, const std::function<void()>& onSuccess,
...
FaceModule::formatItemXml(std::ostream& os, const FaceStatus& item) const
{
    ...
    if (item.hasExpirationPeriod()) {
        os << "<expirationPeriod>" << xml::formatDuration(item.getExpirationPeriod()) << "</expirationPeriod>";
    }
...
FaceModule::formatItemText(std::ostream& os, const FaceStatus& item, bool wantMultiLine)
{
    ...
    os << ia("counters") << "{in={" << item.getNInInterests() << "i " << item.getNInData() << "d " << item.getNInNacks()
       << "n " << item.getNInBytes() << "B} "
       << "out={" << item.getNOutInterests() << "i " << item.getNOutData() << "d " << item.getNOutNacks() << "n "
       << item.getNOutBytes() << "B}}";
