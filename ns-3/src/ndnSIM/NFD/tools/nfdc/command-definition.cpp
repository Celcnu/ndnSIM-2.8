...
CommandDefinition&
CommandDefinition::addArg(const std::string& name, ArgValueType valueType, Required isRequired,
                          Positional allowPositional, const std::string& metavar)
{
    bool isNew = m_args
                   .emplace(name, Arg{name, valueType, static_cast<bool>(isRequired),
                                      metavar.empty() ? getMetavarFromType(valueType) : metavar})
                   .second;
    BOOST_VERIFY(isNew);
        ...
        if (namedArg != m_args.end() && namedArg->second.valueType != ArgValueType::ANY) {
            ...
            if (arg.valueType == ArgValueType::NONE) {
                ...
                try {
                    ...
                    NDN_THROW_NESTED(Error(arg.name + ": cannot parse '" + valueToken + "' as " + arg.metavar + " ("
                                           + e.what() + ")"));
                }
        ...
        for (; positionalArgIndex < m_positionalArgs.size(); ++positionalArgIndex) {
            ...
            try {
                ...
                if (arg.isRequired) { // the current token must be parsed as the value for arg
                    ...
                    NDN_THROW_NESTED(Error("cannot parse '" + token + "' as an argument name or as " + arg.metavar
                                           + " for " + arg.name + " (" + e.what() + ")"));
                }
                else {
