...
UnixStreamChannel::UnixStreamChannel(const unix_stream::Endpoint& endpoint, bool wantCongestionMarking)
  : m_endpoint(endpoint)
...
void
UnixStreamChannel::listen(const FaceCreatedCallback& onFaceCreated, const FaceCreationFailedCallback& onAcceptFailed,
                          int backlog /* = acceptor::max_connections*/)
{
    ...
    if (type == fs::socket_file) {
        ...
        else if (error == boost::asio::error::connection_refused || error == boost::asio::error::timed_out) {
...
void
UnixStreamChannel::accept(const FaceCreatedCallback& onFaceCreated, const FaceCreationFailedCallback& onAcceptFailed)
{
    m_acceptor.async_accept(m_socket, [=](const auto& e) { this->handleAccept(e, onFaceCreated, onAcceptFailed); });
}

void
UnixStreamChannel::handleAccept(const boost::system::error_code& error, const FaceCreatedCallback& onFaceCreated,
