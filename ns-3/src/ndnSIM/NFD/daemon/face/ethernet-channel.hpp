...
class EthernetChannel : public Channel {
  public:
     ...
     */
    void connect(const ethernet::Address& remoteEndpoint, const FaceParams& params,
  ...
  private:
    void asyncRead(const FaceCreatedCallback& onFaceCreated, const FaceCreationFailedCallback& onReceiveFailed);

    void handleRead(const boost::system::error_code& error, const FaceCreatedCallback& onFaceCreated,
                    const FaceCreationFailedCallback& onReceiveFailed);

    void
    processIncomingPacket(const uint8_t* packet, size_t length, const ethernet::Address& sender,
                          const FaceCreatedCallback& onFaceCreated, const FaceCreationFailedCallback& onReceiveFailed);

    std::pair<bool, shared_ptr<Face>> createFace(const ethernet::Address& remoteEndpoint, const FaceParams& params);
