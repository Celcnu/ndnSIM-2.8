...
MulticastUdpTransport::MulticastUdpTransport(const protocol::endpoint& multicastGroup, protocol::socket&& recvSocket,
...
void
MulticastUdpTransport::openRxSocket(protocol::socket& sock, const protocol::endpoint& multicastGroup,
                                    ...
                                    const shared_ptr<const ndn::net::NetworkInterface>& netif)
{
    ...
    if (multicastGroup.address().is_v4()) {
        ...
        sock.set_option(boost::asio::ip::multicast::join_group(multicastGroup.address().to_v4(), localAddress.to_v4()));
    }
    else {
...
void
MulticastUdpTransport::openTxSocket(protocol::socket& sock, const protocol::endpoint& localEndpoint,
                                    const shared_ptr<const ndn::net::NetworkInterface>& netif, bool enableLoopback)
{
    BOOST_ASSERT(!sock.is_open());
...
{
    if (ep.address().is_v4()) {
        return (static_cast<uint64_t>(ep.port()) << 32) | static_cast<uint64_t>(ep.address().to_v4().to_ulong());
    }
    else {
        size_t seed = 0;
