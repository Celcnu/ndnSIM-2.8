...
LpReliability::handleOutgoing(std::vector<lp::Packet>& frags, lp::Packet&& pkt, bool isInterest)
{
    ...
    for (lp::Packet& frag : frags) {
        ...
        unackedFragsIt = m_unackedFrags.emplace_hint(unackedFragsIt, std::piecewise_construct,
                                                     std::forward_as_tuple(txSeq), std::forward_as_tuple(frag));
        unackedFragsIt->second.sendTime = sendTime;
        unackedFragsIt->second.rtoTimer =
          getScheduler().schedule(m_rttEst.getEstimatedRto(), [=] { onLpPacketLost(txSeq); });
...
LpReliability::piggyback(lp::Packet& pkt, ssize_t mtu)
{
    ...
    ssize_t reservedSpace = tlv::sizeOfVarNumber(ndn::MAX_NDN_PACKET_SIZE) - tlv::sizeOfVarNumber(pktSize);
    ...
    while (!m_ackQueue.empty()) {
        ...
        const ssize_t ackSize =
          tlv::sizeOfVarNumber(lp::tlv::Ack) + tlv::sizeOfVarNumber(sizeof(lp::Sequence)) + sizeof(lp::Sequence);
...
LpReliability::onLpPacketLost(lp::Sequence txSeq)
{
    ...
    if (txFrag.retxCount >= m_options.maxRetx) {
        ...
        auto newTxFragIt =
          m_unackedFrags.emplace_hint(m_firstUnackedFrag != m_unackedFrags.end() && m_firstUnackedFrag->first > newTxSeq
                                        ? m_firstUnackedFrag
                                        : m_unackedFrags.end(),
                                      std::piecewise_construct, std::forward_as_tuple(newTxSeq),
        ...
        *fragInNetPkt = newTxFragIt;
        ...
        newTxFrag.rtoTimer = getScheduler().schedule(m_rttEst.getEstimatedRto(), [=] { onLpPacketLost(newTxSeq); });
    }
