...
class LpReliability : noncopyable {
  public:
    ...
    PUBLIC_WITH_TESTS_ELSE_PRIVATE : class UnackedFrag;
    class NetPkt;
     ...
     */
    std::vector<lp::Sequence> onLpPacketLost(lp::Sequence txSeq);
    ...
    PUBLIC_WITH_TESTS_ELSE_PRIVATE :
      ...
      class UnackedFrag {
      public:
        explicit UnackedFrag(lp::Packet pkt);
  ...
  public:
    ...
    static constexpr size_t RESERVED_HEADER_SPACE =
      tlv::sizeOfVarNumber(lp::tlv::TxSequence) + tlv::sizeOfVarNumber(sizeof(lp::Sequence)) + sizeof(lp::Sequence);

    PUBLIC_WITH_TESTS_ELSE_PRIVATE : Options m_options;
