...
class DatagramTransport : public Transport {
  public:
     ...
     */
    explicit DatagramTransport(typename protocol::socket&& socket);
...
DatagramTransport<T, U>::doSend(const Block& packet, const EndpointId&)
{
    ...
    m_socket.async_send(boost::asio::buffer(packet),
                        ...
                        [this, packet](auto&&... args) { this->handleSend(std::forward<decltype(args)>(args)...); });
}
...
DatagramTransport<T, U>::handleReceive(const boost::system::error_code& error, size_t nBytesReceived)
{
    ...
    if (m_socket.is_open())
        m_socket.async_receive_from(boost::asio::buffer(m_receiveBuffer), m_sender, [this](auto&&... args) {
...
DatagramTransport<T, U>::processErrorCode(const boost::system::error_code& error)
{
    ...
    if (getState() == TransportState::CLOSING || getState() == TransportState::FAILED
        || getState() == TransportState::CLOSED || error == boost::asio::error::operation_aborted) {
