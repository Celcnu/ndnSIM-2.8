...
void
EthernetFactory::doProcessConfig(OptionalConfigSection configSection, FaceSystem::ConfigContext& context)
{
    // ether
    // {
    //   listen yes
        ...
        for (const auto& pair : *configSection) {
            ...
            if (key == "listen") {
                ...
                if (mcastConfig.group.isNull()) {
                    NDN_THROW(ConfigFile::Error("face_system.ether.mcast_group: '" + valueStr
    ...
    if (m_mcastConfig.isEnabled != mcastConfig.isEnabled) {
        ...
        if (m_mcastConfig.group != mcastConfig.group) {
            NFD_LOG_INFO("changing multicast group from " << m_mcastConfig.group << " to " << mcastConfig.group);
        }
...
void
EthernetFactory::doCreateFace(const CreateFaceRequest& req, const FaceCreatedCallback& onCreated,
...
shared_ptr<Face>
EthernetFactory::createMulticastFace(const ndn::net::NetworkInterface& netif, const ethernet::Address& address)
{
