...
{
    m_netifAddConn =
      netmon->onInterfaceAdded.connect([this](const auto& netif) { this->applyMcastConfigToNetif(netif); });
}

void
UdpFactory::doProcessConfig(OptionalConfigSection configSection, FaceSystem::ConfigContext& context)
{
    // udp
    // {
    //   listen yes
    ...
    if (configSection) {
        ...
        for (const auto& pair : *configSection) {
            ...
            if (key == "listen") {
                ...
                if (ec) {
                    NDN_THROW(ConfigFile::Error("face_system.udp.mcast_group: '" + valueStr
                                                + "' cannot be parsed as an IPv4 address"));
                }
                else if (!mcastConfig.group.address().is_multicast()) {
                    NDN_THROW(
                      ConfigFile::Error("face_system.udp.mcast_group: '" + valueStr + "' is not a multicast address"));
                }
            }
                ...
                if (ec) {
                    NDN_THROW(ConfigFile::Error("face_system.udp.mcast_group_v6: '" + valueStr
    ...
    if (m_mcastConfig.isEnabled != mcastConfig.isEnabled) {
        ...
        if (m_mcastConfig.group != mcastConfig.group) {
            NFD_LOG_INFO("changing IPv4 multicast group from " << m_mcastConfig.group << " to " << mcastConfig.group);
        }
        if (m_mcastConfig.groupV6 != mcastConfig.groupV6) {
            NFD_LOG_INFO("changing IPv6 multicast group from " << m_mcastConfig.groupV6 << " to "
                                                               << mcastConfig.groupV6);
        }
...
void
UdpFactory::doCreateFace(const CreateFaceRequest& req, const FaceCreatedCallback& onCreated,
...
UdpFactory::createChannel(const udp::Endpoint& localEndpoint, time::nanoseconds idleTimeout)
{
...
shared_ptr<Face>
UdpFactory::createMulticastFace(const shared_ptr<const net::NetworkInterface>& netif, const ip::address& localAddress,
                                const udp::Endpoint& multicastEndpoint)
{
    ...
    auto transport =
      make_unique<MulticastUdpTransport>(mcastEp, std::move(rxSock), std::move(txSock), m_mcastConfig.linkType);
...
UdpFactory::applyMcastConfigToNetif(const shared_ptr<const net::NetworkInterface>& netif)
{
    ...
    for (const auto& addr : addrs) {
        auto face = this->createMulticastFace(netif, addr, addr.is_v4() ? m_mcastConfig.group : m_mcastConfig.groupV6);
...
UdpFactory::applyMcastConfig(const FaceSystem::ConfigContext&)
{
    ...
    boost::copy(m_mcastFaces | boost::adaptors::map_values, std::inserter(facesToClose, facesToClose.end()));
