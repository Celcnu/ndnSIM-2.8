...
class Transport : protected virtual TransportCounters, noncopyable {
  ...
  private: // to be overridden by subclass
     ...
     */
    virtual void doSend(const Block& packet, const EndpointId& endpoint) = 0;
...
template <typename T>
typename std::enable_if<std::is_base_of<Transport, T>::value && !std::is_same<Transport, T>::value, std::ostream&>::type
