...
class EthernetFactory : public ProtocolFactory {
  public:
    ...
    explicit EthernetFactory(const CtorParams& params);
    ...
    createChannel(const shared_ptr<const ndn::net::NetworkInterface>& localEndpoint, time::nanoseconds idleTimeout);
     ...
     */
    shared_ptr<Face>
    createMulticastFace(const ndn::net::NetworkInterface& localEndpoint, const ethernet::Address& group);
  ...
  private:
     ...
     */
    shared_ptr<EthernetChannel> applyUnicastConfigToNetif(const shared_ptr<const ndn::net::NetworkInterface>& netif);
     ...
     */
    shared_ptr<Face> applyMcastConfigToNetif(const ndn::net::NetworkInterface& netif);

    void applyConfig(const FaceSystem::ConfigContext& context);
