...
class UdpChannel : public Channel {
  public:
     ...
     */
    void connect(const udp::Endpoint& remoteEndpoint, const FaceParams& params,
  ...
  private:
    void waitForNewPeer(const FaceCreatedCallback& onFaceCreated, const FaceCreationFailedCallback& onReceiveFailed);

    /**
     * \brief The channel has received a packet from a remote
     *        endpoint not associated with any UDP face yet
     */
    void handleNewPeer(const boost::system::error_code& error, size_t nBytesReceived,
                       const FaceCreatedCallback& onFaceCreated, const FaceCreationFailedCallback& onReceiveFailed);

    std::pair<bool, shared_ptr<Face>> createFace(const udp::Endpoint& remoteEndpoint, const FaceParams& params);
