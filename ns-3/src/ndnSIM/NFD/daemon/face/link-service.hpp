...
class LinkService : protected virtual LinkServiceCounters, noncopyable {
  ...
  private: // upper interface to be overridden in subclass (send path entrypoint)
     ...
     */
    virtual void doSendInterest(const Interest& interest, const EndpointId& endpoint) = 0;

    /** \brief performs LinkService specific operations to send a Data to \p endpoint
     */
    virtual void doSendData(const Data& data, const EndpointId& endpoint) = 0;

    /** \brief performs LinkService specific operations to send a Nack to \p endpoint
     */
    virtual void doSendNack(const lp::Nack& nack, const EndpointId& endpoint) = 0;

  private: // lower interface to be overridden in subclass
    virtual void doReceivePacket(const Block& packet, const EndpointId& endpoint) = 0;
...
template <typename T>
typename std::enable_if<std::is_base_of<LinkService, T>::value && !std::is_same<LinkService, T>::value,
                        std::ostream&>::type
