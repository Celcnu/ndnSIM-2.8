...
#endif
{
    ...
    m_netifStateConn = localEndpoint.onStateChanged.connect(
      [=](ndn::net::InterfaceState, ndn::net::InterfaceState newState) { handleNetifStateChange(newState); });
...
EthernetTransport::doClose()
{
    ...
    getGlobalIoService().post([this] { this->setState(TransportState::CLOSED); });
}
...
EthernetTransport::sendPacket(const ndn::Block& block)
{
    ...
    if (sent < 0)
        ...
        handleError("Failed to send the full frame: size=" + to_string(buffer.size()) + " sent=" + to_string(sent));
    else
...
{
    m_socket.async_read_some(boost::asio::null_buffers(), [this](const auto& e, auto) { this->handleRead(e); });
}
...
EthernetTransport::handleRead(const boost::system::error_code& error)
{
    if (error) {
        ...
        if (error != boost::asio::error::operation_aborted && getState() != TransportState::CLOSING
            && getState() != TransportState::FAILED && getState() != TransportState::CLOSED) {
...
void
EthernetTransport::receivePayload(const uint8_t* payload, size_t length, const ethernet::Address& sender)
{
