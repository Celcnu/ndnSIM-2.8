...
class DeadNonceList : noncopyable {
  public:
     ...
     */
    explicit DeadNonceList(time::nanoseconds lifetime = DEFAULT_LIFETIME);
  ...
  private: // Entry and Index
    ...
    static Entry makeEntry(const Name& name, uint32_t nonce);

    typedef boost::multi_index_container<
      Entry, boost::multi_index::indexed_by<boost::multi_index::sequenced<>,
  ...
  private:
    ...
    PUBLIC_WITH_TESTS_ELSE_PRIVATE : // actual lifetime estimation and capacity control
                                      ...
                                      *
                                      *  The capacity is adjusted so that every Entry is expected to be kept for
                                      * m_lifetime. This is achieved by mark() and adjustCapacity().
                                      */
                                     size_t m_capacity;
