...
StrategyChoice::insert(const Name& prefix, const Name& strategyName)
{
    ...
    if (entry != nullptr) {
        ...
        NFD_LOG_TRACE("insert(" << prefix << ") changing from " << oldStrategy->getInstanceName() << " to "
...
operator<<(std::ostream& os, const StrategyChoice::InsertResult& res)
{
    switch (res.m_status) {
        ...
        case StrategyChoice::InsertResult::DEPTH_EXCEEDED:
            return os << "Prefix has too many components (limit is " << to_string(NameTree::getMaxDepth()) << ")";
    }
    return os;
}
...
StrategyChoice::changeStrategy(Entry& entry, Strategy& oldStrategy, Strategy& newStrategy)
{
    ...
    if (Strategy::areSameType(oldInstanceName, newInstanceName)) {
        ...
        NFD_LOG_INFO("changeStrategy(" << entry.getPrefix() << ") " << oldInstanceName << " -> " << newInstanceName
                                       << " same-type");
        return;
    }
    ...
    const auto& ntChanged =
      m_nameTree.partialEnumerate(entry.getPrefix(), [&rootNte](const name_tree::Entry& nte) -> std::pair<bool, bool> {
          if (&nte == rootNte) {
              return {true, true};
          }
...
StrategyChoice::getRange() const
{
    return m_nameTree.fullEnumerate(&nteHasStrategyChoiceEntry)
           | boost::adaptors::transformed(name_tree::GetTableEntry<Entry>(&name_tree::Entry::getStrategyChoiceEntry));
}
