...
class RibManager : public ManagerBase {
  public:
    RibManager(rib::Rib& rib, ndn::Face& face, ndn::KeyChain& keyChain, ndn::nfd::Controller& nfdController,
  ...
  public: // self-learning support
     ...
     *  announcement.
     */
    void slAnnounce(const ndn::PrefixAnnouncement& pa, uint64_t faceId, time::milliseconds maxLifetime,
  ...
  private: // RIB and FibUpdater actions
    ...
    static SlAnnounceResult getSlAnnounceResultFromRibUpdateResult(RibUpdateResult r);
     ...
     */
    void beginAddRoute(const Name& name, rib::Route route, optional<time::nanoseconds> expires,
     ...
     *  \param done completion callback
     */
    void beginRemoveRoute(const Name& name, const rib::Route& route, const std::function<void(RibUpdateResult)>& done);

    void beginRibUpdate(const rib::RibUpdate& update, const std::function<void(RibUpdateResult)>& done);

  private: // management Dispatcher related
    void registerTopPrefix(const Name& topPrefix);

    /** \brief Serve rib/register command.
     */
    void registerEntry(const Name& topPrefix, const Interest& interest, ControlParameters parameters,
                       const ndn::mgmt::CommandContinuation& done);

    /** \brief Serve rib/unregister command.
     */
    void unregisterEntry(const Name& topPrefix, const Interest& interest, ControlParameters parameters,
                         const ndn::mgmt::CommandContinuation& done);
    ...
    ndn::mgmt::Authorization makeAuthorization(const std::string& verb) override;

  private: // Face monitor
    void fetchActiveFaces();

    void onFetchActiveFacesFailure(uint32_t code, const std::string& reason);

    PUBLIC_WITH_TESTS_ELSE_PRIVATE : void scheduleActiveFaceFetch(const time::seconds& timeToWait);

    void removeInvalidFaces(const std::vector<ndn::nfd::FaceStatus>& activeFaces);
