...
RibManager::RibManager(rib::Rib& rib, ndn::Face& face, ndn::KeyChain& keyChain, ndn::nfd::Controller& nfdController,
...
RibManager::enableLocalFields()
{
    m_nfdController.start<ndn::nfd::FaceUpdateCommand>(
      ...
      [](const ControlParameters&) { NFD_LOG_DEBUG("Local fields enabled"); },
      [](const ControlResponse& res) {
          NDN_THROW(Error("Couldn't enable local fields (" + to_string(res.getCode()) + " " + res.getText() + ")"));
      });
}
                          ...
                          const std::function<void(RibUpdateResult)>& done)
{
    ...
    NFD_LOG_INFO("Adding route " << name << " nexthop=" << route.faceId << " origin=" << route.origin
                                 << " cost=" << route.cost);
...
void
RibManager::beginRemoveRoute(const Name& name, const Route& route, const std::function<void(RibUpdateResult)>& done)
{
    NFD_LOG_INFO("Removing route " << name << " nexthop=" << route.faceId << " origin=" << route.origin);
...
void
RibManager::beginRibUpdate(const RibUpdate& update, const std::function<void(RibUpdateResult)>& done)
{
    m_rib.beginApplyUpdate(
      update,
      [=] {
...
RibManager::registerTopPrefix(const Name& topPrefix)
{
    ...
    m_nfdController.start<ndn::nfd::FibAddNextHopCommand>(
      ...
      [=](const ControlResponse& res) {
          NDN_THROW(Error("Cannot add FIB entry " + topPrefix.toUri() + " (" + to_string(res.getCode()) + " "
                          + res.getText() + ")"));
      });
...
void
RibManager::registerEntry(const Name& topPrefix, const Interest& interest, ControlParameters parameters,
                          const ndn::mgmt::CommandContinuation& done)
{
    if (parameters.getName().size() > Fib::getMaxDepth()) {
        done(ControlResponse(414, "Route prefix cannot exceed " + to_string(Fib::getMaxDepth()) + " components"));
        return;
    }
    ...
    if (parameters.hasExpirationPeriod() && parameters.getExpirationPeriod() != time::milliseconds::max()) {
...
void
RibManager::unregisterEntry(const Name& topPrefix, const Interest& interest, ControlParameters parameters,
                            const ndn::mgmt::CommandContinuation& done)
{
    setFaceForSelfRegistration(interest, parameters);
...
void
RibManager::listEntries(const Name& topPrefix, const Interest& interest, ndn::mgmt::StatusDatasetContext& context)
{
...
{
    return [this](const Name& prefix, const Interest& interest, const ndn::mgmt::ControlParameters* params,
        ...
        validator.validate(interest, bind([&interest, this, accept] { extractRequester(interest, accept); }),
...
void
RibManager::slAnnounce(const ndn::PrefixAnnouncement& pa, uint64_t faceId, time::milliseconds maxLifetime,
                       const SlAnnounceCallback& cb)
{
    BOOST_ASSERT(pa.getData());

    m_paValidator.validate(
      *pa.getData(),
      [=](const Data&) {
          Route route(pa, faceId);
          route.expires = std::min(route.annExpires, time::steady_clock::now() + maxLifetime);
          beginAddRoute(pa.getAnnouncedName(), route, nullopt, [=](RibUpdateResult ribRes) {
      ...
      [=](const Data&, ndn::security::v2::ValidationError err) {
          NFD_LOG_INFO("slAnnounce " << pa.getAnnouncedName() << " " << faceId << " validation error: " << err);
          cb(SlAnnounceResult::VALIDATION_FAILURE);
      });
}

void
RibManager::slRenew(const Name& name, uint64_t faceId, time::milliseconds maxLifetime, const SlAnnounceCallback& cb)
{
    Route routeQuery;
    ...
    route.expires = std::min(route.annExpires, time::steady_clock::now() + maxLifetime);
    beginAddRoute(routeName, route, nullopt, [=](RibUpdateResult ribRes) {
        auto res = getSlAnnounceResultFromRibUpdateResult(ribRes);
...
RibManager::fetchActiveFaces()
{
    ...
    m_nfdController.fetch<ndn::nfd::FaceDataset>(bind(&RibManager::removeInvalidFaces, this, _1),
