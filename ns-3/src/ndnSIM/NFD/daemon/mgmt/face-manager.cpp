...
FaceManager::FaceManager(FaceSystem& faceSystem, Dispatcher& dispatcher, CommandAuthenticator& authenticator)
  ...
  , m_faceTable(faceSystem.getFaceTable())
{
    ...
    m_faceRemoveConn = m_faceTable.beforeRemove.connect(
      [this](const Face& face) { notifyFaceEvent(face, ndn::nfd::FACE_EVENT_DESTROYED); });
}

void
FaceManager::createFace(const ControlParameters& parameters, const ndn::mgmt::CommandContinuation& done)
{
    FaceUri remoteUri;
    ...
    try {
        factory->createFace(
          {remoteUri, localUri, faceParams},
          [this, parameters, done](const auto& face) { this->afterCreateFaceSuccess(face, parameters, done); },
...
makeUpdateFaceResponse(const Face& face)
{
    ...
    params.setFaceId(face.getId()).setFacePersistency(face.getPersistency());
...
makeCreateFaceResponse(const Face& face)
{
    ...
    params.setUri(face.getRemoteUri().toString()).setLocalUri(face.getLocalUri().toString());
...
void
FaceManager::afterCreateFaceSuccess(const shared_ptr<Face>& face, const ControlParameters& parameters,
...
updateLinkServiceOptions(Face& face, const ControlParameters& parameters)
{
    ...
    if (parameters.hasFlagBit(ndn::nfd::BIT_LOCAL_FIELDS_ENABLED) && face.getScope() == ndn::nfd::FACE_SCOPE_LOCAL) {
...
void
FaceManager::updateFace(const Interest& interest, const ControlParameters& parameters,
                        const ndn::mgmt::CommandContinuation& done)
{
...
void
FaceManager::destroyFace(const ControlParameters& parameters, const ndn::mgmt::CommandContinuation& done)
{
...
makeFaceStatus(const Face& face, const time::steady_clock::TimePoint& now)
{
    ...
    if (expirationTime != time::steady_clock::TimePoint::max()) {
        status.setExpirationPeriod(std::max(0_ms, time::duration_cast<time::milliseconds>(expirationTime - now)));
    }
...
{
    if (filter.hasFaceId() && filter.getFaceId() != static_cast<uint64_t>(face.getId())) {
        return false;
    }

    if (filter.hasUriScheme() && filter.getUriScheme() != face.getRemoteUri().getScheme()
        && filter.getUriScheme() != face.getLocalUri().getScheme()) {
        return false;
    }

    if (filter.hasRemoteUri() && filter.getRemoteUri() != face.getRemoteUri().toString()) {
        return false;
    }

    if (filter.hasLocalUri() && filter.getLocalUri() != face.getLocalUri().toString()) {
        return false;
    }

    if (filter.hasFaceScope() && filter.getFaceScope() != face.getScope()) {
        return false;
    }

    if (filter.hasFacePersistency() && filter.getFacePersistency() != face.getPersistency()) {
        return false;
    }

    if (filter.hasLinkType() && filter.getLinkType() != face.getLinkType()) {
        return false;
    }
...
void
FaceManager::queryFaces(const Interest& interest, ndn::mgmt::StatusDatasetContext& context)
{
...
FaceManager::connectFaceStateChangeSignal(const Face& face)
{
    ...
    m_faceStateChangeConn[faceId] =
      face.afterStateChange.connect([this, faceId, &face](FaceState oldState, FaceState newState) {
