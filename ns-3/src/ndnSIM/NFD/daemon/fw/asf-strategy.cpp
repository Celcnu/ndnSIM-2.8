  ...
  , m_retxSuppression(RETX_SUPPRESSION_INITIAL, RetxSuppressionExponential::DEFAULT_MULTIPLIER, RETX_SUPPRESSION_MAX)
{
    ...
    if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
        NDN_THROW(std::invalid_argument("AsfStrategy does not support version " + to_string(*parsed.version)));
    }
...
void
AsfStrategy::beforeSatisfyInterest(const shared_ptr<pit::Entry>& pitEntry, const FaceEndpoint& ingress,
                                   const Data& data)
{
    ...
    if (outRecord == pitEntry->out_end()) {
        ...
        NFD_LOG_DEBUG(pitEntry->getName() << " data from=" << ingress << " rtt=" << faceInfo->getLastRtt()
                                          << " srtt=" << faceInfo->getSrtt());
    }
...
void
AsfStrategy::afterReceiveNack(const FaceEndpoint& ingress, const lp::Nack& nack, const shared_ptr<pit::Entry>& pitEntry)
{
                             ...
                             const shared_ptr<pit::Entry>& pitEntry, bool wantNewNonce)
{
    ...
    if (!faceInfo.isTimeoutScheduled()) {
        auto timeout =
          faceInfo.scheduleTimeout(interest.getName(), [this, name = interest.getName(), faceId = egress.face.getId()] {
              onTimeout(name, faceId);
          });
        NFD_LOG_TRACE("Scheduled timeout for " << fibEntry.getPrefix() << " to=" << egress << " in "
                                               << time::duration_cast<time::milliseconds>(timeout) << " ms");
    }
}
...
Face*
AsfStrategy::getBestFaceForForwarding(const Interest& interest, const Face& inFace, const fib::Entry& fibEntry,
                                      const shared_ptr<pit::Entry>& pitEntry, bool isInterestNew)
{
    ...
    for (const auto& nh : fibEntry.getNextHops()) {
        ...
        if (info == nullptr) {
            rankedFaces.insert(
              {&nh.getFace(), FaceInfo::RTT_NO_MEASUREMENT, FaceInfo::RTT_NO_MEASUREMENT, nh.getCost()});
        }
        else {
