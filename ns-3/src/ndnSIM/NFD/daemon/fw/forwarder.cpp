  ...
  , m_csFace(face::makeNullFace(FaceUri("contentstore://")))
{
  ...
  // 让 m_faceTable 的 afterAdd 信号装配操作(即使得每次新添加的 face 的各信号能连接到对应的 Forwarder 的函数/管道)
  ...
  // 给 m_faceTable 的 beforeRemove 信号装配操作： cleanupOnFaceRemoval （每次移除 face 前先 cleanup ）
...
Forwarder::onIncomingInterest(const FaceEndpoint& ingress, const Interest& interest)
{
  ...
  // 检查m_deadNonceList里有没有与当前包一样的内容, 如果有, 说明interest循环了(环路), 执行this->onInterestLoop
  ...
  // PIT insert
  // 尝试将interest插入PIT表(如果在PIT找到了该interest，自然就不必插入了? 不需要看不同的接口吗? interest里面标识了不同的入口)
  ...
  // 原因在于: 如果PIT匹配到了,说明本地CS肯定没有,可以节省开销,直接不用查了.
...
// CS缓存命中将会触发
// 不过以 ndn-simple.cpp 为例,因为包里面一个Payload的 1024B 里是随机的垃圾数据,所以基本上不可能出现 CS 命中
                             ...
                             const Interest& interest, const Data& data)
{
  ...
  pitEntry->dataFreshnessPeriod = data.getFreshnessPeriod(); // 更新dataFreshnessPeriod --> 因为它又被重新使用了,变得更新鲜了?
...
Forwarder::onOutgoingData(const Data& data, const FaceEndpoint& egress)
{
  ...
  // send Data
  egress.face.sendData(data, egress.endpoint); // TODO: 这个sendData才是最复杂的! 需要调用底层的ns-3实现
  ++m_counters.nOutData;
}
