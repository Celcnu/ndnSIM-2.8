                    ...
                    std::initializer_list<FaceEndpoint> exceptFaceEndpoints)
{
    ...
    // warning: don't loop on pitEntry->getInRecords(), because in-record is deleted when sending
    // Nack
}
...
Strategy::lookupFib(const pit::Entry& pitEntry) const
{
    ...
    // Forwarding hint should have been stripped by incoming Interest pipeline when reaching
    // producer region
