...
main(int argc, char** argv)
{
    ...
    description.add_options()("help,h", "print this message and exit")(
      "version,V", "show version information and exit")("config,c", po::value<std::string>(&configFile),
    ...
    const std::string boostBuildInfo = "with Boost version " + to_string(BOOST_VERSION / 100000) + "."
                                       + to_string(BOOST_VERSION / 100 % 1000) + "." + to_string(BOOST_VERSION % 100);
...
#ifdef HAVE_WEBSOCKET
      "with WebSocket++ version " + to_string(websocketpp::major_version) + "." + to_string(websocketpp::minor_version)
    ...
    std::clog << "NFD version " << NFD_VERSION_BUILD_STRING << " starting\n"
              << "Built with " BOOST_COMPILER ", with " BOOST_STDLIB ", " << boostBuildInfo << ", " << pcapBuildInfo
              << ", " << wsBuildInfo << ", with ndn-cxx version " NDN_CXX_VERSION_BUILD_STRING << std::endl;
