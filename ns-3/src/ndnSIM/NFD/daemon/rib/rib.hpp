...
class Rib : noncopyable {
  ...
  public:
     ...
     *  be updated.
     */
    void beginApplyUpdate(const RibUpdate& update, const UpdateSuccessCallback& onSuccess,
  ...
  private:
    void enqueueRemoveFace(const RibEntry& entry, uint64_t faceId);

    /** \brief Append the RIB update to the update queue.
     *
     *  To start updates, invoke sendBatchFromQueue() .
     */
    void addUpdateToQueue(const RibUpdate& update, const Rib::UpdateSuccessCallback& onSuccess,
     ...
     */
    void sendBatchFromQueue();

    void onFibUpdateSuccess(const RibUpdateBatch& batch, const RibUpdateList& inheritedRoutes,
    ...
    PUBLIC_WITH_TESTS_ELSE_PRIVATE : void erase(const Name& prefix, const Route& route);
  ...
  private:
    ...
    RibTable::iterator eraseEntry(RibTable::iterator it);

    void updateRib(const RibUpdateBatch& batch);

    /** \brief returns routes inherited from the entry's ancestors
     *
     *  \return{ a list of inherited routes }
     */
    RouteSet getAncestorRoutes(const RibEntry& entry) const;
