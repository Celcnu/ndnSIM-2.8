...
Readvertise::Readvertise(Rib& rib, unique_ptr<ReadvertisePolicy> policy, unique_ptr<ReadvertiseDestination> destination)
...
Readvertise::afterAddRoute(const RibRouteRef& ribRoute)
{
    ...
    if (!isNew && rrIt->signer != action->signer) {
        NFD_LOG_WARN("add-route " << ribRoute.entry->getName() << '(' << ribRoute.route->faceId << ','
                                  << ribRoute.route->origin << ") readvertising-as " << action->prefix << " old-signer "
                                  << rrIt->signer << " new-signer " << action->signer);
    }
    ...
    NFD_LOG_DEBUG("add-route " << ribRoute.entry->getName() << '(' << ribRoute.route->faceId << ','
                               << ribRoute.route->origin << ") readvertising-as " << action->prefix << " signer "
                               << action->signer);
...
Readvertise::advertise(ReadvertisedRouteContainer::iterator rrIt)
{
    ...
    m_destination->advertise(
      *rrIt,
      [=] {
          NFD_LOG_DEBUG("advertise " << rrIt->prefix << " success");
          rrIt->retryDelay = RETRY_DELAY_MIN;
          rrIt->retryEvt =
            getScheduler().schedule(randomizeTimer(m_policy->getRefreshInterval()), [=] { advertise(rrIt); });
      },
      [=](const std::string& msg) {
          NFD_LOG_DEBUG("advertise " << rrIt->prefix << " failure " << msg);
          rrIt->retryDelay = std::min(RETRY_DELAY_MAX, rrIt->retryDelay * 2);
          rrIt->retryEvt = getScheduler().schedule(randomizeTimer(rrIt->retryDelay), [=] { advertise(rrIt); });
      });
}
    ...
    m_destination->withdraw(
      *rrIt,
      ...
      [=](const std::string& msg) {
          ...
          rrIt->retryDelay = std::min(RETRY_DELAY_MAX, rrIt->retryDelay * 2);
          rrIt->retryEvt = getScheduler().schedule(randomizeTimer(rrIt->retryDelay), [=] { withdraw(rrIt); });
      });
}
