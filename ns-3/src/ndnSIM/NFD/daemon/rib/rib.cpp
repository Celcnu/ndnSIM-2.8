...
Rib::onRouteExpiration(const Name& prefix, const Route& route)
{
    ...
    RibUpdate update;
    update.setAction(RibUpdate::UNREGISTER).setName(prefix).setRoute(route);
...
void
Rib::beginApplyUpdate(const RibUpdate& update, const Rib::UpdateSuccessCallback& onSuccess,
...
Rib::enqueueRemoveFace(const RibEntry& entry, uint64_t faceId)
{
    for (const Route& route : entry) {
        ...
        RibUpdate update;
        update.setAction(RibUpdate::REMOVE_FACE).setName(entry.getName()).setRoute(route);
        addUpdateToQueue(update, nullptr, nullptr);
    }
}

void
Rib::addUpdateToQueue(const RibUpdate& update, const Rib::UpdateSuccessCallback& onSuccess,
                      const Rib::UpdateFailureCallback& onFailure)
{
...
void
Rib::onFibUpdateSuccess(const RibUpdateBatch& batch, const RibUpdateList& inheritedRoutes,
...
void
Rib::onFibUpdateFailure(const Rib::UpdateFailureCallback& onFailure, uint32_t code, const std::string& error)
{
    m_isUpdateInProgress = false;
