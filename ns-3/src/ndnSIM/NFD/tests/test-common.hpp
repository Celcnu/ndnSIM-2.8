...
shared_ptr<Interest> makeInterest(const Name& name, bool canBePrefix = false,
                                  optional<time::milliseconds> lifetime = nullopt, optional<uint32_t> nonce = nullopt);
 ...
 */
lp::Nack makeNack(Interest interest, lp::NackReason reason);
 ...
 */
ndn::PrefixAnnouncement makePrefixAnn(const Name& announcedName, time::milliseconds expiration,
 ...
 */
ndn::PrefixAnnouncement signPrefixAnn(ndn::PrefixAnnouncement&& pa, ndn::KeyChain& keyChain,
