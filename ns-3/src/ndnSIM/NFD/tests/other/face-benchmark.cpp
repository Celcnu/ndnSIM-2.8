...
class FaceBenchmark {
  ...
  private:
    ...
    {
        std::clog << "Left face created: remote=" << faceL->getRemoteUri() << " local=" << faceL->getLocalUri()
                  << std::endl;
    ...
    {
        std::clog << "Right face created: remote=" << faceR->getRemoteUri() << " local=" << faceR->getLocalUri()
                  << std::endl;
    ...
    {
        face1->afterReceiveInterest.connect(
          [face2](const Interest& interest, const EndpointId&) { face2->sendInterest(interest, 0); });
        face1->afterReceiveData.connect([face2](const Data& data, const EndpointId&) { face2->sendData(data, 0); });
        face1->afterReceiveNack.connect(
          [face2](const ndn::lp::Nack& nack, const EndpointId&) { face2->sendNack(nack, 0); });
    }
