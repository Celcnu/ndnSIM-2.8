...
class FaceSystemFixture : public virtual GlobalIoFixture {
  public:
    ...
    listFacesByScheme(const std::string& scheme, ndn::nfd::LinkType linkType = ndn::nfd::LINK_TYPE_NONE) const
    {
        std::vector<const Face*> faces;
        for (const Face& face : faceTable) {
            if ((face.getLocalUri().getScheme() == scheme || face.getRemoteUri().getScheme() == scheme)
                && (linkType == ndn::nfd::LINK_TYPE_NONE || face.getLinkType() == linkType)) {
...
class DummyProtocolFactory : public ProtocolFactory {
  public:
    ...
    void
    doProcessConfig(OptionalConfigSection configSection, FaceSystem::ConfigContext& context) final
    {
        processConfigHistory.push_back({configSection, context.isDryRun, context.generalConfig.wantCongestionMarking});
        if (!context.isDryRun) {
