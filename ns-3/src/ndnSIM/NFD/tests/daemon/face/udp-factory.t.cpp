...
BOOST_AUTO_TEST_CASE(DisableListen)
{
    ...
    auto channels = factory.getChannels();
    BOOST_CHECK(std::none_of(channels.begin(), channels.end(), [](const auto& ch) { return ch->isListening(); }));
}
...
BOOST_FIXTURE_TEST_CASE(CreateChannel, UdpFactoryMcastFixture)
{
    ...
    if (!netifsV4.empty()) {
        ...
        BOOST_CHECK_EXCEPTION(createChannel("127.0.0.1", 20072), UdpFactory::Error, [](const UdpFactory::Error& e) {
            return strcmp(e.what(), "Cannot create UDP channel on 127.0.0.1:20072, "
    ...
    if (!netifsV6.empty()) {
        ...
        BOOST_CHECK_EXCEPTION(createChannel("::1", 20072), UdpFactory::Error, [](const UdpFactory::Error& e) {
            return strcmp(e.what(), "Cannot create UDP channel on [::1]:20072, "
                                    "endpoint already allocated to a UDP multicast face")
                   == 0;
        });
    }
...
BOOST_FIXTURE_TEST_CASE(CreateMulticastFaceV4, UdpFactoryMcastFixture)
{
    ...
    BOOST_CHECK_EXCEPTION(createMulticastFace("127.0.0.1", "224.0.0.254", 20071), UdpFactory::Error,
                          [](const UdpFactory::Error& e) {
                              return strcmp(e.what(), "Cannot create UDP multicast face on 127.0.0.1:20071, "
    ...
    BOOST_CHECK_EXCEPTION(createMulticastFace("127.0.0.1", "224.0.0.42", 20070), UdpFactory::Error,
                          [](const UdpFactory::Error& e) {
                              return strcmp(e.what(), "Cannot create UDP multicast face on 127.0.0.1:20070, "
...
BOOST_FIXTURE_TEST_CASE(CreateMulticastFaceV6, UdpFactoryMcastFixture)
{
    ...
    BOOST_CHECK_EXCEPTION(createMulticastFace("::1", "ff02::114", 20071), UdpFactory::Error,
                          [](const UdpFactory::Error& e) {
                              return strcmp(e.what(), "Cannot create UDP multicast face on [::1]:20071, "
                                                      "endpoint already allocated to a UDP channel")
                                     == 0;
                          });
    ...
    BOOST_CHECK_EXCEPTION(createMulticastFace("::1", "ff02::42", 20070), UdpFactory::Error,
                          [](const UdpFactory::Error& e) {
                              return strcmp(e.what(), "Cannot create UDP multicast face on [::1]:20070, "
                                                      "endpoint already allocated to a different UDP multicast face")
                                     == 0;
                          });
}
...
{
    createFace(factory, FaceUri("udp4://127.0.0.1:6363"), {},
    ...
    createFace(factory, FaceUri("udp4://127.0.0.1:20072"), {},
               {ndn::nfd::FACE_PERSISTENCY_PERMANENT, {}, {}, {}, false, false, false},
               {CreateFaceExpectedResult::SUCCESS, 0, ""});

    createFace(factory, FaceUri("udp4://127.0.0.1:20073"), {},
    ...
    createFace(factory, FaceUri("udp4://127.0.0.1:20074"), {},
...
BOOST_AUTO_TEST_CASE(UnsupportedCreateFace)
{
    ...
    createFace(factory, FaceUri("udp4://127.0.0.1:20072"), FaceUri("udp4://127.0.0.1:20071"),
               {ndn::nfd::FACE_PERSISTENCY_PERSISTENT, {}, {}, {}, false, false, false},
               {CreateFaceExpectedResult::FAILURE, 406, "Unicast UDP faces cannot be created with a LocalUri"});

    createFace(factory, FaceUri("udp4://127.0.0.1:20072"), {},
               {ndn::nfd::FACE_PERSISTENCY_ON_DEMAND, {}, {}, {}, false, false, false},
               {CreateFaceExpectedResult::FAILURE, 406, "Outgoing UDP faces do not support on-demand persistency"});

    createFace(factory, FaceUri("udp4://233.252.0.1:23252"), {},
               {ndn::nfd::FACE_PERSISTENCY_PERSISTENT, {}, {}, {}, false, false, false},
               {CreateFaceExpectedResult::FAILURE, 406, "Cannot create multicast UDP faces"});

    createFace(factory, FaceUri("udp4://127.0.0.1:20072"), {},
               {ndn::nfd::FACE_PERSISTENCY_PERSISTENT, {}, {}, {}, true, false, false},
               {CreateFaceExpectedResult::FAILURE, 406, "Local fields can only be enabled on faces with local scope"});
}
