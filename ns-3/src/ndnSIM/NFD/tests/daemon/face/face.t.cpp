...
BOOST_AUTO_TEST_CASE(LinkServiceSendReceive)
{
    ...
    for (size_t i = 0; i < nInNacks; ++i) {
        face1->receiveNack(makeNack(*makeInterest("/StnEVTj4Ex", false, nullopt, 561), lp::NackReason::CONGESTION), 0);
    }
    ...
    for (size_t i = 0; i < nOutNacks; ++i) {
        face1->sendNack(makeNack(*makeInterest("/9xK6FbwIBM", false, nullopt, 365), lp::NackReason::CONGESTION), 0);
    }
