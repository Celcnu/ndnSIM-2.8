...
BOOST_AUTO_TEST_CASE(FragmentSingleFragment)
{
    ...
    packet.add<lp::FragmentField>(std::make_pair(data->wireEncode().begin(), data->wireEncode().end()));
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(data->wireEncode().begin(), data->wireEncode().end(), fragBegin, fragEnd);
}
...
BOOST_AUTO_TEST_CASE(FragmentMultipleFragments)
{
    ...
    BOOST_REQUIRE_LE(std::distance(frag1Begin, frag1End), std::distance(reassembledPos, reassembledPayload.end()));
    ...
    BOOST_REQUIRE_LE(std::distance(frag2Begin, frag2End), std::distance(reassembledPos, reassembledPayload.end()));
    ...
    BOOST_REQUIRE_LE(std::distance(frag3Begin, frag3End), std::distance(reassembledPos, reassembledPayload.end()));
    ...
    BOOST_REQUIRE_LE(std::distance(frag4Begin, frag4End), std::distance(reassembledPos, reassembledPayload.end()));
    std::copy(frag4Begin, frag4End, reassembledPos);

    BOOST_CHECK_EQUAL_COLLECTIONS(data->wireEncode().begin(), data->wireEncode().end(), reassembledPayload.begin(),
                                  reassembledPayload.end());
}
