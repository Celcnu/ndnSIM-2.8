...
class UnixStreamChannelFixture : public ChannelFixture<UnixStreamChannel, unix_stream::Endpoint> {
  protected:
    ...
    {
        client.async_connect(listenerEp, [this](const boost::system::error_code& error) {
...
BOOST_AUTO_TEST_CASE(SocketFile)
{
    ...
    BOOST_CHECK_EQUAL(status.permissions(), fs::owner_read | fs::group_read | fs::others_read | fs::owner_write
