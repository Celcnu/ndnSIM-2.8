...
using UnicastUdpTransportFixtures =
  boost::mpl::vector<GENERATE_IP_TRANSPORT_FIXTURE_INSTANTIATIONS(UnicastUdpTransportFixture)>;
...
using RemoteCloseFixture = IpTransportFixture<UnicastUdpTransportFixture, AddressFamily::Any, AddressScope::Loopback>;
using RemoteClosePersistencies = boost::mpl::vector_c<ndn::nfd::FacePersistency, ndn::nfd::FACE_PERSISTENCY_ON_DEMAND,
...
BOOST_FIXTURE_TEST_CASE(RemoteClosePermanent, RemoteCloseFixture)
{
    ...
    remoteSocket.async_receive(boost::asio::buffer(readBuf), [this](const boost::system::error_code& error, size_t) {
