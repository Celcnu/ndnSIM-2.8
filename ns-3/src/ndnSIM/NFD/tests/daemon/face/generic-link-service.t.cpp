...
class GenericLinkServiceFixture : public GlobalIoTimeFixture, public KeyChainFixture {
  protected:
    ...
    {
        face =
          make_unique<Face>(make_unique<GenericLinkService>(options),
                            make_unique<DummyTransport>("dummy://", "dummy://", ndn::nfd::FACE_SCOPE_NON_LOCAL,
                                                        ndn::nfd::FACE_PERSISTENCY_PERSISTENT,
                                                        ndn::nfd::LINK_TYPE_POINT_TO_POINT, mtu, sendQueueCapacity));
        ...
        face->afterReceiveData.connect([this](const Data& data, const EndpointId&) { receivedData.push_back(data); });
    ...
    {
        return lp::PrefixAnnouncementHeader{
          signPrefixAnn(makePrefixAnn(announcedName, 1_h), m_keyChain, ndn::signingWithSha256())};
    }
...
BOOST_AUTO_TEST_CASE(SendNack)
{
    ...
    auto nack1 = makeNack(*makeInterest("/localhost/test", false, nullopt, 323), lp::NackReason::NO_ROUTE);
...
BOOST_AUTO_TEST_CASE(ReceiveInterest)
{
    ...
    auto interest1 = makeInterest("/23Rd9hEiR");
    lp::Packet lpPacket;
    lpPacket.set<lp::FragmentField>(std::make_pair(interest1->wireEncode().begin(), interest1->wireEncode().end()));
...
BOOST_AUTO_TEST_CASE(ReceiveData)
{
    ...
    lp::Packet lpPacket;
    lpPacket.set<lp::FragmentField>(std::make_pair(data1->wireEncode().begin(), data1->wireEncode().end()));
    lpPacket.set<lp::SequenceField>(0); // force LpPacket encoding
...
BOOST_AUTO_TEST_CASE(ReceiveNack)
{
    ...
    lp::Nack nack1 = makeNack(*makeInterest("/localhost/test", false, nullopt, 323), lp::NackReason::NO_ROUTE);
    lp::Packet lpPacket;
    lpPacket.set<lp::FragmentField>(
      std::make_pair(nack1.getInterest().wireEncode().begin(), nack1.getInterest().wireEncode().end()));
...
BOOST_AUTO_TEST_CASE(CongestionCoDel)
{
    ...
    time::nanoseconds markingInterval(static_cast<time::nanoseconds::rep>(
      options.baseCongestionMarkingInterval.count() / std::sqrt(service->m_nMarkedSinceInMarkingState + 1)));
    ...
    markingInterval = time::nanoseconds(static_cast<time::nanoseconds::rep>(
      options.baseCongestionMarkingInterval.count() / std::sqrt(service->m_nMarkedSinceInMarkingState + 1)));
...
BOOST_AUTO_TEST_CASE(ReceiveNextHopFaceIdDropNack)
{
    ...
    lp::Nack nack = makeNack(*makeInterest("/localhost/test", false, nullopt, 123), lp::NackReason::NO_ROUTE);
    lp::Packet packet;
    packet.set<lp::FragmentField>(
      std::make_pair(nack.getInterest().wireEncode().begin(), nack.getInterest().wireEncode().end()));
...
BOOST_AUTO_TEST_CASE(ReceiveIncomingFaceIdIgnoreNack)
{
    ...
    lp::Nack nack = makeNack(*makeInterest("/TPAhdiHz", false, nullopt, 278), lp::NackReason::CONGESTION);
    lp::Packet packet(nack.getInterest().wireEncode());
    packet.set<lp::NackField>(nack.getHeader());
    packet.set<lp::IncomingFaceIdField>(1000);
