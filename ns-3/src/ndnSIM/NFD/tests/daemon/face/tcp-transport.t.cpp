...
using TcpTransportFixtures = boost::mpl::vector<GENERATE_IP_TRANSPORT_FIXTURE_INSTANTIATIONS(TcpTransportFixture)>;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(StaticProperties, T, TcpTransportFixtures, T)
{
    ...
    BOOST_CHECK_EQUAL(this->transport->getScope(), this->addressScope == AddressScope::Loopback
...
BOOST_AUTO_TEST_CASE(ChangePersistencyFromPermanentWhenDown)
{
    ...
    transport->afterStateChange.connectSingleShot([&didStateChange](auto oldState, auto newState) {
        didStateChange = true;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(PermanentReconnectWithExponentialBackoff, T, TcpTransportFixtures, T)
{
    ...
    auto transportObserver = make_unique<PermanentTcpTransportReconnectObserver>(std::move(sock), this->limitedIo);
    ...
    auto expectedWait2 =
      time::duration_cast<time::nanoseconds>(expectedWait1 * TcpTransport::s_reconnectWaitMultiplier);
    ...
    BOOST_CHECK_CLOSE(asFloatMilliseconds(retryTime2 - retryTime1), asFloatMilliseconds(expectedWait1),
                      20.0); // 200ms tolerance
    BOOST_CHECK_CLOSE(asFloatMilliseconds(retryTime3 - retryTime2), asFloatMilliseconds(expectedWait2),
                      10.0); // 200ms tolerance

    // reestablish the TCP connection
    this->startAccept(remoteEp);

    auto expectedWait3 =
      time::duration_cast<time::nanoseconds>(expectedWait2 * TcpTransport::s_reconnectWaitMultiplier);
    ...
    BOOST_CHECK_CLOSE(asFloatMilliseconds(retryTime5 - retryTime4), asFloatMilliseconds(expectedWait1),
                      20.0); // 200ms tolerance
}
