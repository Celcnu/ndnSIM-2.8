...
class DummyManager : public ManagerBase {
  public:
    explicit DummyManager(Dispatcher& dispatcher)
  ...
  private:
    ...
    {
        return [](const Name& prefix, const Interest& interest, const ndn::mgmt::ControlParameters* params,
                  ndn::mgmt::AcceptContinuation accept, ndn::mgmt::RejectContinuation reject) { accept("requester"); };
    }
};
...
BOOST_AUTO_TEST_CASE(RegisterNotificationStream)
{
    ...
    BOOST_CHECK_EQUAL(m_responses[0].getName(), Name("/localhost/nfd/test-module/test-notification/%FE%00"));
}
