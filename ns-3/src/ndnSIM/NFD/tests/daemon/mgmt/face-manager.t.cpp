...
BOOST_AUTO_TEST_CASE(Existing)
{
    ...
    BOOST_CHECK_EQUAL(checkResponse(1, req.getName(), makeResponse(200, "OK", parameters)), CheckResponseResult::OK);
...
BOOST_AUTO_TEST_CASE(NonExisting)
{
    ...
    BOOST_CHECK_EQUAL(checkResponse(0, req.getName(), makeResponse(200, "OK", parameters)), CheckResponseResult::OK);
}
...
BOOST_AUTO_TEST_CASE(FaceDataset)
{
    ...
    BOOST_CHECK_EQUAL(status.hasExpirationPeriod(), face->getExpirationTime() != time::steady_clock::time_point::max());
...
BOOST_AUTO_TEST_CASE(FaceQuery)
{
    ...
    auto generateQuery = [](const FaceQueryFilter& filter) {
        return Interest(Name("/localhost/nfd/faces/query").append(filter.wireEncode())).setCanBePrefix(true);
    };
    ...
    auto invalidQueryName = Name("/localhost/nfd/faces/query").append(ndn::makeStringBlock(tlv::Content, "invalid"));
...
class TestChannel : public face::Channel {
  public:
    explicit TestChannel(const std::string& uri)
    {
        setUri(FaceUri(uri));
    }
