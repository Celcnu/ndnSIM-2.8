...
class FaceUriMalformed {
  public:
    ...
    getParameters()
    {
        return ControlParameters().setUri("tcp4://127.0.0.1:not-a-port");
    }
};
...
class FaceUriNonCanonical {
  public:
    ...
    getParameters()
    {
        return ControlParameters().setUri("udp://localhost");
    }
};
...
class FaceUriUnsupportedScheme {
  public:
    ...
    getParameters()
    {
        return ControlParameters().setUri("dev://eth0");
    }
};
...
using TestCases = mpl::vector<
  mpl::pair<TcpFaceOnDemand, CommandFailure<406>>, mpl::pair<TcpFacePersistent, CommandSuccess>,
  mpl::pair<TcpFacePermanent, CommandSuccess>, mpl::pair<UdpFaceOnDemand, CommandFailure<406>>,
  mpl::pair<UdpFacePersistent, CommandSuccess>, mpl::pair<UdpFacePermanent, CommandSuccess>,
  mpl::pair<LocalTcpFaceLocalFieldsEnabled, CommandSuccess>, mpl::pair<LocalTcpFaceLocalFieldsDisabled, CommandSuccess>,
  mpl::pair<NonLocalUdpFaceLocalFieldsEnabled, CommandFailure<406>>,
  mpl::pair<NonLocalUdpFaceLocalFieldsDisabled, CommandSuccess>, mpl::pair<TcpFaceLpReliabilityEnabled, CommandSuccess>,
  mpl::pair<TcpFaceLpReliabilityDisabled, CommandSuccess>, mpl::pair<UdpFaceLpReliabilityEnabled, CommandSuccess>,
  mpl::pair<UdpFaceLpReliabilityDisabled, CommandSuccess>, mpl::pair<TcpFaceCongestionMarkingEnabled, CommandSuccess>,
  mpl::pair<TcpFaceCongestionMarkingDisabled, CommandSuccess>, mpl::pair<TcpFaceMtuOverride, CommandFailure<406>>,
  mpl::pair<UdpFaceMtuOverride, CommandSuccess>, mpl::pair<FaceUriMalformed, CommandFailure<400>>,
  mpl::pair<FaceUriNonCanonical, CommandFailure<400>>, mpl::pair<FaceUriUnsupportedScheme, CommandFailure<406>>>;
    ...
    this->node1.face.onSendData.connect([this, req, &hasCallbackFired](const Data& response) {
        ...
        if (actual.getCode() != 200) {
            ...
            if (uri.parse(FaceType::getParameters().getUri())) {
                ...
                BOOST_CHECK(std::none_of(faceTable.begin(), faceTable.end(),
                                         [uri](Face& face) { return face.getRemoteUri() == uri; }));
            }
            else {
...
BOOST_FIXTURE_TEST_CASE(ExistingFace, FaceManagerCommandFixture)
{
...

    {
        ...
        bool hasCallbackFired = false;
        this->node1.face.onSendData.connect([req, foundFace, &hasCallbackFired](const Data& response) {
            if (!req.getName().isPrefixOf(response.getName())) {
                return;
            }
