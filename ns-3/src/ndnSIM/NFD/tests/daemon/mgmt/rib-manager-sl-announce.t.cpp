...
class RibManagerSlAnnounceFixture : public GlobalIoTimeFixture, public KeyChainFixture {
  public:
    ...
    slAnnounceSync(const ndn::PrefixAnnouncement& pa, uint64_t faceId, time::milliseconds maxLifetime)
    {
        ...
        manager->slAnnounce(pa, faceId, maxLifetime, [&](RibManager::SlAnnounceResult res) {
            BOOST_CHECK(!result);
            result = res;
        });
    ...
    {
        optional<SlAnnounceResult> result;
        manager->slRenew(name, faceId, maxLifetime, [&](RibManager::SlAnnounceResult res) {
            BOOST_CHECK(!result);
            result = res;
        });
    ...
    slFindAnnSync(const Name& name)
    {
        ...
        manager->slFindAnn(name, [&](optional<ndn::PrefixAnnouncement> found) {
            BOOST_CHECK(!result);
            result = found;
        });
