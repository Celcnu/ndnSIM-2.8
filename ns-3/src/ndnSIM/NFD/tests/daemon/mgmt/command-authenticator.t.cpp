...
class CommandAuthenticatorFixture : public CommandInterestSignerFixture {
  protected:
    ...
    {
        Interest interest =
          this->makeControlCommandRequest(Name("/prefix/" + module + "/verb"), ControlParameters(), identity);
        ...
        BOOST_REQUIRE_MESSAGE(isAccepted || isRejected, "authorization function should invoke one continuation");
        return isAccepted;
    }
...
{
    BOOST_CHECK_EQUAL(authorize1([](Interest& interest) { interest.setName("/prefix"); }), false);
    BOOST_CHECK(lastRejectReply == ndn::mgmt::RejectReply::SILENT);
}

BOOST_AUTO_TEST_CASE(BadKeyLocator_BadSigInfo)
{
    BOOST_CHECK_EQUAL(authorize1([](Interest& interest) {
...
BOOST_AUTO_TEST_CASE(BadKeyLocator_MissingKeyLocator)
{
    BOOST_CHECK_EQUAL(authorize1([](Interest& interest) {
                          ...
                          setNameComponent(interest, ndn::signed_interest::POS_SIG_INFO, sigInfo.wireEncode().begin(),
                                           sigInfo.wireEncode().end());
                      }),
                      false);
    BOOST_CHECK(lastRejectReply == ndn::mgmt::RejectReply::SILENT);
}
...
BOOST_AUTO_TEST_CASE(BadKeyLocator_BadKeyLocatorType)
{
    BOOST_CHECK_EQUAL(authorize1([](Interest& interest) {
                          ndn::KeyLocator kl;
                          kl.setKeyDigest(
                            ndn::encoding::makeBinaryBlock(tlv::KeyDigest, "\xDD\xDD\xDD\xDD\xDD\xDD\xDD\xDD", 8));
                          ndn::SignatureInfo sigInfo(tlv::SignatureSha256WithRsa);
...
BOOST_AUTO_TEST_CASE(InvalidTimestamp)
{
    ...
    BOOST_CHECK_EQUAL(authorize1([&timestampComp](const Interest& interest) {
                          timestampComp = interest.getName().at(ndn::command_interest::POS_TIMESTAMP);
                      }),
                      true); // accept first command
    BOOST_CHECK_EQUAL(authorize1([&timestampComp](Interest& interest) {
