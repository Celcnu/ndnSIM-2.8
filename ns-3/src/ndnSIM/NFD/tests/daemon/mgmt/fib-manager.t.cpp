...
class FibManagerFixture : public ManagerFixtureWithAuthenticator {
  ...
  public: // for check
    enum class CheckNextHopResult { OK, NO_FIB_ENTRY, WRONG_N_NEXTHOPS, NO_NEXTHOP, WRONG_COST };
     ...
     */
    CheckNextHopResult
    checkNextHop(const Name& prefix, optional<size_t> expectedNNextHops = nullopt, optional<FaceId> faceId = nullopt,
...
{
    auto req =
      makeControlCommandRequest("/localhost/nfd/fib/add-nexthop", makeParameters("hello", face::FACEID_NULL, 101));
    receiveInterest(req);
    BOOST_REQUIRE_EQUAL(m_responses.size(), 1);

    // check response
    BOOST_CHECK_EQUAL(checkResponse(0, req.getName(), ControlResponse(410, "Face not found")), CheckResponseResult::OK);
...
BOOST_AUTO_TEST_CASE(NameTooLong)
{
    ...
    auto req = makeControlCommandRequest("/localhost/nfd/fib/add-nexthop", makeParameters(prefix, addFace()));
    receiveInterest(req);

    ControlResponse expected(414,
                             "FIB entry prefix cannot exceed " + ndn::to_string(Fib::getMaxDepth()) + " components");
...
BOOST_AUTO_TEST_CASE(FibDataset)
{
    ...
    for (size_t idx = 0; idx < nEntries; ++idx) {
        ...
        for (const auto& nh : matchedEntry->getNextHops()) {
            expectedRecords.back().addNextHopRecord(
              ndn::nfd::NextHopRecord().setFaceId(nh.getFace().getId()).setCost(nh.getCost()));
        }
    }
