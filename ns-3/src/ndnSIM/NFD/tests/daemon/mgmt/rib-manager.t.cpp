...
class RibManagerFixture : public ManagerCommonFixture {
  public:
      ...
      , m_manager(m_rib, m_face, m_keyChain, m_nfdController, m_dispatcher)
    {
        ...
        ndn::security::SigningInfo signingInfo(ndn::security::SigningInfo::SIGNER_TYPE_ID, m_anchorId, signatureInfo);
  ...
  public:
    ...
    makeRegisterParameters(const Name& name, uint64_t faceId = 0, time::milliseconds expiry = time::milliseconds::max())
    {
    ...
    {
        return ControlParameters().setName(name).setFaceId(faceId).setOrigin(ndn::nfd::ROUTE_ORIGIN_NLSR);
    }
...
using AllFixtures = boost::mpl::vector<UnauthorizedRibManagerFixture, LocalhostAuthorizedRibManagerFixture,
                                       LocalhopAuthorizedRibManagerFixture, AuthorizedRibManagerFixture>;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(CommandAuthorization, T, AllFixtures, T)
{
    ...
    auto commandHop = this->makeControlCommandRequest("/localhop/nfd/rib/register", parameters, this->m_derivedId);
...
BOOST_AUTO_TEST_CASE(Expiration)
{
    ...
    BOOST_CHECK_EQUAL(m_fibUpdater.updates.front(), rib::FibUpdate::createAddUpdate("/test-expiry", 9527, 10));
    BOOST_CHECK_EQUAL(m_fibUpdater.updates.back(), rib::FibUpdate::createRemoveUpdate("/test-expiry", 9527));
...
BOOST_AUTO_TEST_CASE(NameTooLong)
{
    ...
    BOOST_CHECK_EQUAL(checkResponse(0, command.getName(),
                                    ControlResponse(414, "Route prefix cannot exceed " + to_string(Fib::getMaxDepth())
                                                           + " components")),
                      CheckResponseResult::OK);
...
BOOST_FIXTURE_TEST_CASE(RibDataset, UnauthorizedRibManagerFixture)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(receivedRecords.begin(), receivedRecords.end(), expectedRecords.begin(),
                                  expectedRecords.end());
}
