...
class FaceManagerUpdateFixture : public FaceManagerCommandFixture {
  public:
    ...
    createFace(const std::string& uri = "tcp4://127.0.0.1:26363",
               ...
               optional<uint64_t> defaultCongestionThreshold = {}, bool enableLocalFields = false,
    ...
    createFace(const ControlParameters& createParams, bool isForOnDemandFace = false)
    {
        ...
        signal::ScopedConnection connection =
          target.face.onSendData.connect([&, req, isForOnDemandFace, this](const Data& response) {
               ...
               const std::function<void(const ControlResponse& resp)>& checkResp)
    {
        ...
        bool hasCallbackFired = false;
        signal::ScopedConnection connection =
          this->node1.face.onSendData.connect([req, &hasCallbackFired, &checkResp](const Data& response) {
              if (!req.getName().isPrefixOf(response.getName())) {
                  return;
              }
...
using UpdatePersistencyTests = mpl::vector<mpl::pair<DummyTransportBase<true>, CommandSuccess>,
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(UpdatePersistency, T, UpdatePersistencyTests, FaceManagerUpdateFixture)
{
    ...
    auto face = make_shared<face::Face>(make_unique<face::GenericLinkService>(), make_unique<TransportType>());
    this->node1.faceTable.add(face);

    auto parameters =
      ControlParameters().setFaceId(face->getId()).setFacePersistency(ndn::nfd::FACE_PERSISTENCY_PERMANENT);
...
using LocalFieldFaces =
  mpl::vector<mpl::pair<TcpLocalFieldsEnable, CommandSuccess>, mpl::pair<TcpLocalFieldsDisable, CommandSuccess>,
              mpl::pair<UdpLocalFieldsEnable, CommandFailure<409>>, mpl::pair<UdpLocalFieldsDisable, CommandSuccess>,
