...
class CommandInterestSignerFixture : public GlobalIoTimeFixture, public KeyChainFixture {
  protected:
     ...
     */
    Interest makeCommandInterest(const Name& name, const Name& identity = DEFAULT_COMMAND_SIGNER_IDENTITY);
     ...
     *  \return a command Interest
     */
    Interest makeControlCommandRequest(Name commandName, const ControlParameters& params,
...
class ManagerCommonFixture : public CommandInterestSignerFixture {
  ...
  public: // verify
    ControlResponse makeResponse(uint32_t code, const std::string& text, const ControlParameters& parameters);
     ...
     */
    CheckResponseResult checkResponse(size_t idx, const Name& expectedName, const ControlResponse& expectedResponse,
     ...
     */
    Block concatenateResponses(size_t startIndex = 0, size_t nResponses = 0);
...
std::ostream& operator<<(std::ostream& os, ManagerCommonFixture::CheckResponseResult result);
...
class CommandSuccess {
  public:
    ...
    getExpected()
    {
        return ControlResponse().setCode(200).setText("OK");
    }
};
...
class CommandFailure {
  public:
    ...
    getExpected()
    {
        return ControlResponse().setCode(CODE);
