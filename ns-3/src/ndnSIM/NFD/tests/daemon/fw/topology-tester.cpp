...
TopologyLink::addFace(TopologyNode i, shared_ptr<Face> face)
{
    ...
    auto ret = m_transports.emplace(std::piecewise_construct, std::forward_as_tuple(i),
...
TopologyLink::transmit(TopologyNode i, const Block& packet)
{
    ...
    for (const auto& p : m_transports) {
        ...
        getScheduler().schedule(m_delay,
                                [packet, recipient = p.second.transport] { recipient->receivePacket(packet); });
    }
}
...
class TopologyBareLink::Observer : public face::InternalTransportBase {
  public:
    explicit Observer(std::vector<Block>& receivedPackets)
...
class TopologyPcapLinkService : public GenericLinkService, public TopologyPcap {
  private:
...
shared_ptr<Face>
TopologyTester::makeFace(TopologyNode i, const FaceUri& localUri, const FaceUri& remoteUri, ndn::nfd::FaceScope scope,
                         ndn::nfd::LinkType linkType)
{
    unique_ptr<GenericLinkService> service =
      m_wantPcap ? make_unique<TopologyPcapLinkService>() : make_unique<GenericLinkService>();
...
TopologyTester::addLink(const std::string& label, time::nanoseconds delay,
                        std::initializer_list<TopologyNode> forwarders, ndn::nfd::LinkType linkType)
{
    auto link = std::make_shared<TopologyLink>(delay);
    FaceUri remoteUri("topology://link/" + label);
    if (linkType == ndn::nfd::LINK_TYPE_NONE) {
        linkType = forwarders.size() > 2 ? ndn::nfd::LINK_TYPE_MULTI_ACCESS : ndn::nfd::LINK_TYPE_POINT_TO_POINT;
    }
...
TopologyTester::addEchoProducer(ndn::Face& face, const Name& prefix, time::nanoseconds replyDelay)
{
    ...
    face.setInterestFilter(prefix, [=, &face](const auto&, const auto& interest) {
        ...
        if (replyDelay == 0_ns) {
            ...
            // delay the reply
            getScheduler().schedule(replyDelay, [&face, data = std::move(data)] { face.put(*data); });
        }
    });
}

void
TopologyTester::addIntervalConsumer(ndn::Face& face, const Name& prefix, time::nanoseconds interval, size_t n, int seq)
{
    Name name(prefix);
    if (seq >= 0) {
    ...
    if (n > 1) {
        getScheduler().schedule(interval, [=, &face] { addIntervalConsumer(face, prefix, interval, n - 1, seq); });
    }
}
