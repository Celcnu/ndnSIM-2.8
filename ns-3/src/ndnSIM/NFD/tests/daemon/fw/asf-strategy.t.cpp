...
class AsfGridFixture : public GlobalIoTimeFixture {
  protected:
    explicit AsfGridFixture(const Name& params = AsfStrategy::getStrategyName(), time::nanoseconds replyDelay = 0_ms)
      : parameters(params)
    {
        /*
         *                  +---------+
...
class AsfStrategyParametersGridFixture : public AsfGridFixture {
  protected:
    AsfStrategyParametersGridFixture()
      : AsfGridFixture(
        Name(AsfStrategy::getStrategyName()).append("probing-interval~30000").append("n-silent-timeouts~5"))
    {
    }
};
...
BOOST_AUTO_TEST_CASE(Retransmission)
{
    ...
    TopologyNode nodeB = topo.addForwarder("B"), nodeC = topo.addForwarder("C");
...
BOOST_AUTO_TEST_CASE(NoPitOutRecordAndProbeInterestNewNonce)
{
    ...
    TopologyTester topo;
    TopologyNode nodeA = topo.addForwarder("A"), nodeB = topo.addForwarder("B"), nodeC = topo.addForwarder("C"),
                 nodeD = topo.addForwarder("D");
