...
class TopologyLink : noncopyable {
  public:
    explicit TopologyLink(time::nanoseconds delay);
  ...
  private:
    void transmit(TopologyNode i, const Block& packet);
  ...
  private:
    ...
    class ReceiveProxy : public face::InternalTransportBase {
      public:
        ...
        explicit ReceiveProxy(Callback cb)
          : m_cb(std::move(cb))
        {
        }
...
class TopologySingleLink : noncopyable {
  public:
     ...
     */
    explicit TopologySingleLink(shared_ptr<Face> forwarderFace);
...
class TopologyAppLink : public TopologySingleLink {
  public:
     ...
     *  \param forwarderFace a Face with InternalForwarderTransport
     */
    explicit TopologyAppLink(shared_ptr<Face> forwarderFace);
...
class TopologyBareLink : public TopologySingleLink {
  public:
     ...
     *  \param forwarderFace a Face with InternalForwarderTransport
     */
    explicit TopologyBareLink(shared_ptr<Face> forwarderFace);

    void receivePacket(const Block& packet);
...
class TopologyTester : noncopyable {
  public:
     ...
     */
    TopologyNode addForwarder(const std::string& label);
    ...
    template <typename S>
    void
    setStrategy(TopologyNode i, Name prefix = Name("ndn:/"), Name instanceName = S::getStrategyName())
    {
    ...
    addLink(const std::string& label, time::nanoseconds delay, std::initializer_list<TopologyNode> forwarders,
            ndn::nfd::LinkType linkType = ndn::nfd::LINK_TYPE_NONE);

    /** \brief makes a link to local application
     */
    shared_ptr<TopologyAppLink> addAppFace(const std::string& label, TopologyNode i);
    ...
    addBareLink(const std::string& label, TopologyNode i, ndn::nfd::FaceScope scope = ndn::nfd::FACE_SCOPE_LOCAL,
     ...
     */
    TopologyPcap& getPcap(const Face& face);

    /** \brief registers a prefix on a forwarder face
     */
    void registerPrefix(TopologyNode i, const Face& face, const Name& prefix, uint64_t cost = 0);

    /** \brief creates a producer application that answers every Interest with Data of same Name
     */
    void addEchoProducer(ndn::Face& face, const Name& prefix = "/", time::nanoseconds replyDelay = 0_ns);
     ...
     */
    void addIntervalConsumer(ndn::Face& face, const Name& prefix, time::nanoseconds interval, size_t n, int seq = -1);

  private:
    shared_ptr<Face> makeFace(TopologyNode i, const FaceUri& localUri, const FaceUri& remoteUri,
                              ndn::nfd::FaceScope scope, ndn::nfd::LinkType linkType);

  private:
    class TopologyForwarder {
      public:
        explicit TopologyForwarder(const std::string& label)
          : label(label)
        {
        }
