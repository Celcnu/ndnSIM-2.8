...
BOOST_AUTO_TEST_CASE(Forward2)
{
    ...
    std::transform(strategy.sendInterestHistory.begin(), strategy.sendInterestHistory.end(),
                   ...
                   [](const MulticastStrategyTester::SendInterestArgs& args) { return args.outFaceId; });
    ...
    const time::nanoseconds TICK =
      time::duration_cast<time::nanoseconds>(MulticastStrategy::RETX_SUPPRESSION_INITIAL * 0.1);
