...
using Strategies = boost::mpl::vector<BestRouteStrategy2, MulticastStrategy, RandomStrategy>;

// one upstream, send Nack when Nack arrives
BOOST_FIXTURE_TEST_CASE_TEMPLATE(OneUpstream, S, Strategies, StrategyNackReturnFixture<S>)
{
    ...
    auto f = [&] { this->strategy.afterReceiveNack(FaceEndpoint(*this->face3, 0), nack3, pitEntry); };
    ...
    BOOST_CHECK_EQUAL(this->strategy.sendNackHistory[1].pitInterest.wireEncode(), pitEntry->getInterest().wireEncode());
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(nackFaceIds.begin(), nackFaceIds.end(), expectedNackFaceIds.begin(),
                                  expectedNackFaceIds.end());
}

// two upstreams, send Nack when both Nacks arrive
BOOST_FIXTURE_TEST_CASE_TEMPLATE(TwoUpstreams, S, Strategies, StrategyNackReturnFixture<S>)
{
    fib::Entry& fibEntry = *this->fib.insert(Name()).first;
    this->fib.addOrUpdateNextHop(fibEntry, *this->face3, 10);
    this->fib.addOrUpdateNextHop(fibEntry, *this->face4, 20);
    ...
    auto f = [&] { this->strategy.afterReceiveNack(FaceEndpoint(*this->face4, 0), nack4, pitEntry); };
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(Timeout, S, Strategies, StrategyNackReturnFixture<S>)
{
    fib::Entry& fibEntry = *this->fib.insert(Name()).first;
    this->fib.addOrUpdateNextHop(fibEntry, *this->face3, 10);
    this->fib.addOrUpdateNextHop(fibEntry, *this->face4, 20);
...
// #3033 note-7
BOOST_FIXTURE_TEST_CASE_TEMPLATE(LiveDeadlock, S, Strategies, GlobalIoTimeFixture)
{
    /*
     *           /----------\
     *           | producer |
    ...
    TopologyTester topo;
    TopologyNode nodeP = topo.addForwarder("P"), nodeR = topo.addForwarder("R"), nodeA = topo.addForwarder("A"),
                 nodeB = topo.addForwarder("B"), nodeC = topo.addForwarder("C"), nodeD = topo.addForwarder("D");
                                 ...
                                 StrategyNackReturnFixture<BestRouteStrategy2>)
{
    ...
    BOOST_CHECK_EQUAL(strategy.sendNackHistory[0].pitInterest.wireEncode(), pitEntry->getInterest().wireEncode());
