...
class DummySubscriber {
  public:
    ...
    {
        return m_nRemainingACallbacks == 0 && m_nRemainingBCallbacks == 0;
    }

    bool
    noCallbacksFired() const
    {
        return m_nRemainingACallbacks == m_nASections && m_nRemainingBCallbacks == m_nBSections;
    }
...
class DummyAllSubscriber : public DummySubscriber {
  public:
    explicit DummyAllSubscriber(ConfigFile& config, bool expectDryRun = false)
...
class DummyOneSubscriber : public DummySubscriber {
  public:
    DummyOneSubscriber(ConfigFile& config, const std::string& sectionName, bool expectDryRun = false)
      : DummySubscriber(config, (sectionName == "a"), (sectionName == "b"), expectDryRun)
    {
        if (sectionName == "a") {
...
class MissingCallbackFixture {
  public:
    void
    checkMissingHandler(const std::string& filename, const std::string& sectionName, const ConfigSection& section,
                        bool isDryRun)
    {
        m_missingFired = true;
    }
...
BOOST_FIXTURE_TEST_CASE(UncoveredSections, MissingCallbackFixture)
{
    ...
    ConfigFile permissiveFile(bind(&MissingCallbackFixture::checkMissingHandler, this, _1, _2, _3, _4));
