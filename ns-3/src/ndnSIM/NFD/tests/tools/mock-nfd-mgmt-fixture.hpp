...
class MockNfdMgmtFixture : public ClockFixture, public KeyChainFixture {
  protected:
    MockNfdMgmtFixture()
      : ClockFixture(m_io)
      , face(m_io, m_keyChain, {true, false, bind(&MockNfdMgmtFixture::processEventsOverride, this, _1)})
    {
  ...
  private:
    ...
    void
    sendCommandReply(const Interest& interest, uint32_t code, const std::string& text, const Block& body)
    {
...
#define MOCK_NFD_MGMT_REQUIRE_COMMAND_IS(expectedPrefix)                                                               \
    [interest] {                                                                                                       \
        ...
        BOOST_REQUIRE_MESSAGE(params, "Interest " << interest.getName() << " does not match command prefix "           \
                                                  << (expectedPrefix));                                                \
        return *params;                                                                                                \
    }()
