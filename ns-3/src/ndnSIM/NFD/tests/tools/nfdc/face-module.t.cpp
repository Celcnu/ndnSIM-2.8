...
const std::string QUERY_OUTPUT = "faceid=177 remote=tcp4://53.239.9.114:6363 local=tcp4://164.0.31.106:20396"
...
{
    this->processInterest = [this](const Interest& interest) { this->sendEmptyDataset(interest.getName()); };
...
class ExecuteFaceCreateCommandFixture : public ExecuteCommandFixture {
  protected:
    ...
    respond409(const Interest& interest, FacePersistency persistency, optional<uint64_t> mtu = {},
               bool enableLpReliability = false, bool enableCongestionMarking = false)
    {
...
BOOST_AUTO_TEST_CASE(UpgradingPersistency)
{
    ...
    this->processInterest = [this, &hasUpdateCommand](const Interest& interest) {
        ...
        ControlParameters resp;
        resp.setFaceId(1172).setFacePersistency(FacePersistency::FACE_PERSISTENCY_PERSISTENT).setFlags(0);
        this->succeedCommand(interest, resp);
    };
...
{
    this->processInterest = [this](const Interest& interest) { BOOST_CHECK(this->respondFaceQuery(interest)); };
