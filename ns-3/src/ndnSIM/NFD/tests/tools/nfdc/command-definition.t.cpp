...
BOOST_AUTO_TEST_CASE(PositionalArgs)
{
    ...
    BOOST_CHECK_EQUAL(ca.size(), 2);
    BOOST_CHECK_EQUAL(ca.get<FaceUri>("remote"), FaceUri("udp4://router.example.com"));
    BOOST_CHECK_EQUAL(ca.get<FacePersistency>("persistency"), FacePersistency::FACE_PERSISTENCY_PERSISTENT);
    ...
    ca = cs.parse(std::vector<std::string>{"udp4://router.example.com", "persistency", "persistent"});
    BOOST_CHECK_EQUAL(ca.size(), 2);
    BOOST_CHECK_EQUAL(ca.get<FaceUri>("remote"), FaceUri("udp4://router.example.com"));
    BOOST_CHECK_EQUAL(ca.get<FacePersistency>("persistency"), FacePersistency::FACE_PERSISTENCY_PERSISTENT);

    ca = cs.parse(std::vector<std::string>{"remote", "udp4://router.example.com", "persistency", "persistent"});
    BOOST_CHECK_EQUAL(ca.size(), 2);
    BOOST_CHECK_EQUAL(ca.get<FaceUri>("remote"), FaceUri("udp4://router.example.com"));
    BOOST_CHECK_EQUAL(ca.get<FacePersistency>("persistency"), FacePersistency::FACE_PERSISTENCY_PERSISTENT);

    ca = cs.parse(std::vector<std::string>{"persistency", "persistent", "remote", "udp4://router.example.com"});
    BOOST_CHECK_EQUAL(ca.size(), 2);
    BOOST_CHECK_EQUAL(ca.get<FaceUri>("remote"), FaceUri("udp4://router.example.com"));
    BOOST_CHECK_EQUAL(ca.get<FacePersistency>("persistency"), FacePersistency::FACE_PERSISTENCY_PERSISTENT);

    BOOST_CHECK_THROW(cs.parse(std::vector<std::string>{"persistent", "udp4://router.example.com"}),
                      CommandDefinition::Error);
    BOOST_CHECK_THROW(cs.parse(std::vector<std::string>{"persistency", "persistent", "udp4://router.example.com"}),
                      CommandDefinition::Error);
    BOOST_CHECK_THROW(cs.parse(std::vector<std::string>{"remote", "udp4://router.example.com", "persistent"}),
                      CommandDefinition::Error);
}
