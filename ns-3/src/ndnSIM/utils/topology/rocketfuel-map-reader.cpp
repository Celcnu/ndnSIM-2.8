...
#define ROCKETFUEL_MAPS_LINE                                                                                           \
    START "(-*[0-9]+)" SPACE "(@[?A-Za-z0-9,+]+)" SPACE "(\\+)*" MAYSPACE "(bb)*" MAYSPACE "\\(([0-9]+)\\)" SPACE      \
          "(&[0-9]+)*" MAYSPACE "->" MAYSPACE "(<[0-9 \t<>]+>)*" MAYSPACE "(\\{-[0-9\\{\\} \t-]+\\})*" SPACE           \
          "=([A-Za-z0-9.!-]+)" SPACE "r([0-9])" MAYSPACE END

void
RocketfuelMapReader::CreateLink(string nodeName1, string nodeName2, double averageRtt, const string& minBw,
                                const string& maxBw, const string& minDelay, const string& maxDelay)
{
    ...
    DataRate randBandwidth(m_randVar->GetInteger(static_cast<uint32_t>(lexical_cast<DataRate>(minBw).GetBitRate()),
                                                 static_cast<uint32_t>(lexical_cast<DataRate>(maxBw).GetBitRate())));

    int32_t metric =
      std::max(1, static_cast<int32_t>(1.0 * m_referenceOspfRate.GetBitRate() / randBandwidth.GetBitRate()));

    Time randDelay = Time::FromDouble((m_randVar->GetValue(lexical_cast<Time>(minDelay).ToDouble(Time::US),
...
RocketfuelMapReader::GenerateFromMapsFile(int argc, char* argv[])
{
    ...
    if (num_neigh != neigh_list.size()) {
        NS_LOG_WARN("Given number of neighbors = " << num_neigh
...
void
RocketfuelMapReader::assignGw(Traits::vertex_descriptor vertex, uint32_t degree, node_type_t nodeType)
{
...
RocketfuelMapReader::Read(RocketfuelParams params, bool keepOneComponent /*=true*/, bool connectBackbones /*=true*/)
{
    ...
    for (tie(e, ende) = edges(m_graph); e != ende;) {
        ...
        else if ((u_type == GATEWAY && v_type == BACKBONE) || (u_type == BACKBONE && v_type == GATEWAY)) {
            // ok
        }
...
RocketfuelMapReader::SaveTopology(const std::string& file)
{
    ...
    for (std::list<Link>::iterator link = m_linksList.begin(); link != m_linksList.end(); link++) {
        ...
        if (link->GetAttributeFailSafe("OSPF", tmp))
            ...
            int32_t cost =
              std::max(1, static_cast<int32_t>(1.0 * m_referenceOspfRate.GetBitRate() / rate.GetBitRate()));
...
RocketfuelMapReader::ConnectBackboneRouters()
{
    ...
    typedef adjacency_list<
      setS, setS, boost::undirectedS,
      property<vertex_name_t, Traits::vertex_descriptor, property<vertex_index_t, int, property<vertex_index1_t, int>>>>
      BbGraph;
    BbGraph bbGraph;
