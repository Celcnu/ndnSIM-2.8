...
AnnotatedTopologyReader::Read(void)
{
    ...
    while (!topgen.eof()) {
        ...
        if (processedLinks[to].size() != 0 && processedLinks[to].find(from) != processedLinks[to].end()) {
        ...
        AddLink(link);
        NS_LOG_DEBUG("New link " << from << " <==> " << to << " / " << capacity << " with " << metric << " metric ("
...
AnnotatedTopologyReader::SaveTopology(const std::string& file)
{
    ...
    for (std::list<Link>::const_iterator link = m_linksList.begin(); link != m_linksList.end(); link++) {
...
AnnotatedTopologyReader::SaveGraphviz(const std::string& file)
{
    ...
    typedef boost::property<boost::vertex_name_t, std::string, boost::property<boost::vertex_index_t, uint32_t>>
      nodeProperty;

    typedef boost::no_property edgeProperty;

    typedef boost::adjacency_list<boost::setS, boost::setS, boost::undirectedS, nodeProperty, edgeProperty> Graph;
    ...
    for (NodeContainer::Iterator node = m_nodes.Begin(); node != m_nodes.End(); node++) {
        std::pair<node_map_t::iterator, bool> retval =
          graphNodes.insert(make_pair(Names::FindName(*node), add_vertex(nodeProperty(Names::FindName(*node)), graph)));
        // NS_ASSERT (ok == true);
