    ...
    static TypeId tid =
      TypeId("ns3::ndn::RttMeanDeviation")
        ...
        .AddAttribute("Gain", "Gain used in estimating the RTT (smoothed RTT), must be 0 < Gain < 1",
                      DoubleValue(0.125), MakeDoubleAccessor(&RttMeanDeviation::m_gain), MakeDoubleChecker<double>())
        .AddAttribute("Gain2", "Gain2 used in estimating the RTT (variance), must be 0 < Gain2 < 1", DoubleValue(0.25),
                      MakeDoubleAccessor(&RttMeanDeviation::m_gain2), MakeDoubleChecker<double>());
    return tid;
}
...
RttMeanDeviation::RetransmitTimeout()
{
    ...
    double retval =
      std::min(m_maxRto.ToDouble(Time::S),
